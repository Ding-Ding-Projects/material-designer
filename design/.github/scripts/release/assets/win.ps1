...
$sourceInstaller = Join-Path $builderDir "Material Designer-${env:TOOLS_PACK_NAMESPACE}-setup.exe"
$sourceZip = Join-Path $builderDir "Material Designer-${env:TOOLS_PACK_NAMESPACE}-portable.zip"
...
$releaseNotes = if ([string]::IsNullOrWhiteSpace($env:RELEASE_NOTES)) {
  "Material Designer ${env:RELEASE_VERSION}$assetSuffix"
} else {
  $env:RELEASE_NOTES
}
