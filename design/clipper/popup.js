// OpenDesign web clipper popup. Thin UI over the service worker message API.
//
// Zero-config: no pairing. The popup just probes whether OpenDesign is running
...
function render(connected) {
  ...
  // "OpenDesign not running" message instead of being hidden, so it never feels broken.
}
...
$('figma').addEventListener('click', async () => {
  ...
  const res = await send({ type: 'prepareDownloadFigma', opts: captureOpts() });
  reportCapture(res, (r) => {
    if (r.flowId) {
      void chrome.windows.create({
        ...
        try {
          await chrome.windows.update(win.id, { focused: true, alwaysOnTop: true });
        } finally {
          // The service worker asks the browser for the resulting value instead
          // of trusting a self-reported always-on-top flag from this page.
          await send({ type: 'setDownloadWindowState', flowId: r.flowId, windowId: win.id });
        }
      }).catch(() => {
