...
function focusAvailable(root) {
  const target = root?.querySelector('button:not([hidden]):not(:disabled), [href]:not([hidden]), input:not([hidden]):not(:disabled), select:not([hidden]):not(:disabled), textarea:not([hidden]):not(:disabled)');
  target?.focus();
}

function mountDialog(root, onEscape) {
  if (!root) return () => {};
  const onKeyDown = (event) => {
    ...
    const focusable = [...root.querySelectorAll('button:not([hidden]):not(:disabled), [href]:not([hidden]), input:not([hidden]):not(:disabled), select:not([hidden]):not(:disabled), textarea:not([hidden]):not(:disabled)')];
...
function renderState(state) {
  ...
  if (priorState === 'start' && currentState !== 'start') {
    focusAvailable(document.querySelector('.surface'));
  }
}
...
async function runAction(buttonId, message, after = refreshState) {
  ...
  try {
    ...
    if (buttonId === 'start') focusAvailable(document.querySelector('.surface'));
  }
}
...
const disposeDialog = mountDialog(document.querySelector('.surface'), () => {
  if (actionPending) return;
