...
async function runElectronBuilderRawWithPaths(
  ...
  const builderConfig = {
    ...
    win: {
      ...
      icon: paths.winIconPath,
      // electron-builder uses rcedit for executable resource mutation and
      // otherwise reaches its signing path for Squirrel executable targets.
      // This product must not invoke a signer or mutate executable resources
      // through that path. The Squirrel icon asset remains an explicit package
      // input and is validated separately from this executable-processing path.
