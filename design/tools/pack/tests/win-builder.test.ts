...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Material Designer-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Material Designer-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Material Designer"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Material Designer", "Material Designer.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Material Designer-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Material Designer-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Material Designer-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Material Designer", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Material Designer.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Material Designer.exe"))).resolves.toMatchObject({
...
describe("Windows pack artifact boundaries", () => {
  it("uses direct Squirrel output and exposes its machine-readable artifacts", async () => {
    ...
    expect(builderSource).toContain("forceCodeSigning: config.signed");
    expect(builderSource).toContain('CSC_IDENTITY_AUTO_DISCOVERY: config.signed ? "true" : "false"');
...
async function createVersionedExecutable(packagedVersion: string): Promise<Buffer> {
  ...
  version.setStringValues(
    ...
    {
      FileDescription: "Material Designer",
