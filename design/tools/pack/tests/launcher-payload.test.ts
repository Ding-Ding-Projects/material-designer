...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Material Designer-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Material Designer-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Material Designer Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Material Designer Beta", "Material Designer.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Material Designer-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Material Designer-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Material Designer-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Material Designer Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Material Designer.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "Material Designer.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "Material Designer.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "Material Designer Beta.app",
      channel: "beta",
      entry: {
        cwd: "payload/Material Designer Beta.app",
        executable: "payload/Material Designer Beta.app/Contents/MacOS/Material Designer Beta",
      },
      namespace: "release-beta",
      payloadRoot: "payload",
      platform: "darwin",
    ...
    expect(buildWinLauncherPayloadManifest({
      ...
      entry: {
        cwd: "payload",
        executable: "payload/Material Designer.exe",
      },
      namespace: "release-beta-win",
      payloadRoot: "payload",
      platform: "win32",
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("Material Designer Beta.app");
      expect(manifest.entry.executable).toBe("payload/Material Designer Beta.app/Contents/MacOS/Material Designer Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/Material Designer.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "Material Designer.exe"));
