...
export class PasswordProtectedHistory {
  ...
  async prune(beforeMs: number): Promise<number> { const records = await this.records(); const paths = records.filter((record) => Date.parse(record.createdAt) < beforeMs).map((record) => `${record.id}.json`); if (paths.length === 0) return 0; await execFileAsync("git", ["-C", this.#directory, "rm", "--quiet", "--", ...paths]); await execFileAsync("git", ["-C", this.#directory, "-c", "user.name=Material Designer local history", "-c", "user.email=local-history@invalid", "commit", "--quiet", "-m", "Prune local history"]); return paths.length; }
