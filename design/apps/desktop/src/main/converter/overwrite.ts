...
export class OverwriteAuthorizationStore {
  ...
  async #snapshot(path: string): Promise<DestinationSnapshot> {
    if (process.platform !== "win32") return snapshotDestination(path);
    return new WindowsNativeConverterWriter(this.#windowsWriterResourceRoot).inspectChild(path);
  }
  ...
  async issue(request: OverwriteRequest): Promise<OverwriteChallenge> {
    ...
    const destination = await this.#snapshot(request.destinationPath);
  ...
  async consume(token: string, request: OverwriteRequest): Promise<{ expectedDestination: DestinationSnapshot }> {
    ...
    const current = await this.#snapshot(request.destinationPath);
