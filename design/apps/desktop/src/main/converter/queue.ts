...
function normalizeQueueItem(raw: unknown, lineNumber?: number): QueueItem {
  ...
  if (value.requiresDisclosure !== undefined && typeof value.requiresDisclosure !== "boolean") {
    throw new Error(`The durable converter queue contains an invalid disclosure state${suffix}.`);
  }
  return {
    id: value.id,
    adapterId:
    ...
    ...(value.requiresDisclosure === true ? { requiresDisclosure: true } : {}),
  };
}
...
async function* queueExportChunks(
  ...
  maxItems: number,
  maxBytes: number,
  progress: { items: number; bytes: number },
  ...
  for (;;) {
    ...
    for (const item of page.items) {
      ...
      if (progress.items > maxItems || progress.bytes > maxBytes) {
        throw new Error("The queue export exceeded its bounded record or byte limit.");
      }
      yield line;
    }
...
export async function exportQueueToFile(
  ...
  if (process.platform === "win32") {
    ...
    if ((await writer.inspectChild(destination)).exists) {
      throw new Error("The selected queue export destination already exists.");
    }
    const expectedParentIdentity = await writer.inspectParent(dirname(destination));
    await runtime.windowsBeforeLaunch?.();
    const progress = { items: 0, bytes: 0 };
...
export class ConversionQueue {
  ...
  async enqueue(
    ...
    const item: QueueItem = {
      ...
      bytesProcessed: 0,
      ...(totalBytes === undefined ? {} : { totalBytes }),
      ...(requiresDisclosure ? { requiresDisclosure: true } : {}),
      updatedAt: Date.now(),
    };
    ...
    do {
      ...
      for (const item of page.items) {
        if (item.state !== "running" && !(
          item.requiresDisclosure === true
          && (item.state === "queued" || item.state === "paused")
        )) continue;
        item.state = "failed";
        item.reason = item.requiresDisclosure === true
          ? "This queued conversion needs a new loss disclosure review after restart; preview and queue it again."
          : "The previous conversion stopped before completion; retry after reviewing the source and destination.";
