...
describe('RegexSearchField — every field owns its own builder', () => {
  ...
  it('refuses duplicate runtime field ids on initial render instead of sharing storage identity', () => {
    ...
    expect(screen.getByTestId('a').closest('[data-regex-field-id="same-field"]'))
      .toHaveAttribute('data-regex-field-duplicate', 'true');
    expect(screen.getByTestId('b').closest('[data-regex-field-id="same-field"]'))
      .toHaveAttribute('data-regex-field-duplicate', 'true');
