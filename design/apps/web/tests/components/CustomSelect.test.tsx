...
describe('CustomSelect', () => {
  ...
  it('owns the concrete builder root and unregisters it when the portal closes', () => {
    render(
      <CustomSelect
        {...SEARCH_PROPS}
        testId="portal-root"
    ...
    const trigger = screen.getByTestId('portal-root');
    fireEvent.click(trigger);
    fireEvent.click(screen.getByTestId('portal-root-filter-regex-toggle'));
    const popover = screen.getByTestId('portal-root-filter-regex-popover');
    fireEvent.click(screen.getByTestId('portal-root-filter-regex-enable-regex'));
    const pattern = screen.getByTestId('portal-root-filter-regex-pattern');

    dispatchWithoutComposedPath(pattern, 'pointerdown');
    dispatchWithoutComposedPath(pattern, 'mousedown');
    expect(trigger).toHaveAttribute('aria-expanded', 'true');

    fireEvent.click(screen.getByTestId('portal-root-filter-regex-close'));
