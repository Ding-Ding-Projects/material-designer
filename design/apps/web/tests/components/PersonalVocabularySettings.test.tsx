...
const universalState = vi.hoisted(() => ({
  schoolEnabled: false,
  listener: null as ((settings: { school: { enabled: boolean } }) => void) | null,
}));
...
vi.mock('../../src/components/universal/universalSettings', () => ({
  readUniversalSettings: () => ({ school: { enabled: universalState.schoolEnabled } }),
  subscribeUniversalSettings: (listener: (settings: { school: { enabled: boolean } }) => void) => {
...
import { PersonalVocabularySettings } from '../../src/components/PersonalVocabularySettings';
...
describe('PersonalVocabularySettings', () => {
  ...
  it('keeps the last valid cache when a replacement file is invalid', async () => {
    ...
    expect(screen.getByText('Every entry replacement must be a string.')).toBeTruthy();
    expect(window.localStorage.getItem(PERSONAL_VOCABULARY_STORAGE_KEY)).toContain('display');
  });
  ...
  it('suppresses and restores live when the canonical School setting changes', () => {
    ...
    act(() => universalState.listener?.({ school: { enabled: true } }));
    expect(screen.queryByText('Personal wording')).toBeNull();
    act(() => universalState.listener?.({ school: { enabled: false } }));
