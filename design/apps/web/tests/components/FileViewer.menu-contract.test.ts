  ...
  return (() => {
    ...
    const visit = (node: ts.Node) => {
      if (ts.isJsxExpression(node) && node.expression) {
        if (node.expression.getText() === expected) found = true;
        if (ts.isConditionalExpression(node.expression)
          && node.expression.condition.getText() === expected) found = true;
      }
      ts.forEachChild(node, visit);
    };
    visit(fileViewerAst(source));
  ...
  const visit = (node: ts.Node) => {
    ...
    const inspect = (child: ts.Node) => {
      if (ts.isIfStatement(child)
        ...
        && child.expression.expression.text === 'isOwnedRegexBuilder'
        && child.expression.arguments.length === 2
        && child.expression.arguments[1]?.getText() === 'resolvedSurfaceId'
        && ts.isReturnStatement(child.thenStatement)) found = true;
...
describe('FileViewer menu search contract', () => {
  ...
  it('keeps every FileViewer dropdown and history search on a field-owned builder', () => {
    ...
    expect(selectRegistrations.map((registration) => registration.ownerId)).toEqual(
      FILE_VIEWER_SELECT_AND_SEARCH_INVENTORY
        .filter((surface) => surface.id !== 'version-history' && surface.id !== 'html-viewport')
  ...
  it('keeps target actions and destructive operations fail-closed at the owner boundary', () => {
    ...
    // The FileViewer menu primitive requires its caller-owned id, while the
    // shared field keeps an optional legacy-compatible fallback for existing
    // non-FileViewer surfaces that already own stable test/input ids.
    expect(astHasInterfaceProperty(regexFieldSource, 'RegexSearchFieldProps', 'fieldId', true)).toBe(true);
  ...
  it('keeps the regex field bound to the owning menu collection', () => {
    ...
    expect(astHasCallExpression(menuPrimitiveSource, 'escapeSelectorToken', ['ownerToken'])).toBe(true);
  ...
  it('fails closed when a surface, owner token, nested-widget exclusion, or geometry rule disappears', () => {
    ...
    expectRedThenGreenAst(
      ...
      (source) => source.replace('maxHeight: availableHeight', 'height: availableHeight'),
    );
  });
  ...
  it('keeps no-opener, disabled, mixed-focus, portal and inactive-viewer boundaries explicit', () => {
    ...
    expect(astHasCallExpression(menuPrimitiveSource, 'isOwnedTrigger', ['event.target', 'triggerRef'])).toBe(true);
    expect(astHasJsxAttribute(regexFieldSource, 'div', 'data-file-viewer-menu-builder', 'focusScopeId')).toBe(true);
    expect(astHasJsxAttribute(menuPrimitiveSource, 'RegexSearchField', 'focusScopeId', 'resolvedSurfaceId')).toBe(true);
  ...
  it('keeps direct labels wrappable at narrow bilingual widths', () => {
    ...
    expectRedThenGreenAst(
      customSelectSource,
      (source) => astPropertyNameCount(source, 'maxHeight') === 1,
      (source) => source.replace('maxHeight: position.maxHeight', 'height: position.maxHeight'),
    );
