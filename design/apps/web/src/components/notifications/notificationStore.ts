...
for (const record of records) {
  const match = /^od-notification-(\d+)$/u.exec(record.id);
...
/** Permanently remove reviewed records. Callers must place this behind the
 * app's destructive super-confirmation surface. */
export function deleteNotifications(ids: readonly string[]): void {
  const wanted = new Set(ids);
  if (wanted.size === 0) return;
  for (const id of wanted) clearTimer(id);
  const next = records.filter((record) => !wanted.has(record.id));
