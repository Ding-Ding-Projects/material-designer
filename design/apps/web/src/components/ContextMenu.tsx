...
export function ContextMenu({
  ...
  const activate = useCallback((item: ContextMenuItem) => {
    ...
    if (item.danger) {
      ...
      // Close before invoking the action so a synchronous action that replaces
      // its target cannot leave an interactive menu attached to stale content.
      onClose();
      item.onSelect();
    }
          ...
          return (
            <div key={`${item.id}-${index}`} className={styles.row}>
              ...
              <button
                ...
                data-testid={item.testId ?? (testId ? `${testId}-${item.id}` : optionId)}
