...
export function UnlockLadder({ lockoutId, bridge, schoolMode = false, onCleared, labels, copy }: UnlockLadderProps) {
  ...
  const moleVisible = (cell: number) => reducedMotion
    ? staticMoleCells.has(cell)
    : challenge?.moles?.some((mole) => mole.cell === cell && now >= mole.visibleFromMs && now <= mole.visibleUntilMs) ?? false;
  ...
  return (
    <section className={styles.surface} aria-labelledby="unlock-ladder-title" data-testid="unlock-ladder" data-reduced-motion={reducedMotion ? 'true' : 'false'} data-copy-language={copy?.languageMode ?? 'english'} data-copy-fallback={validCopy ? 'false' : 'true'}>
      ...
      {challenge?.stage === 'mole' ? <div className={styles.panel}><p className={styles.status} role="status" aria-live="polite">{activeLabels?.molePrompt ?? `Hit each visible mole once. The round ends after ${Math.ceil((challenge.durationMs ?? 5_000) / 1_000)} seconds. ${remainingSeconds} seconds remain.`}</p>{reducedMotion ? <p className={styles.note} role="status">{activeLabels?.reducedMotionMoleNote ?? 'Reduced motion keeps mole positions still. The numeric countdown and keyboard controls remain available; the host still checks each hit against its visible interval.'}</p> : null}<div className={styles.moles} role="grid" aria-label="Whack-a-mole board" aria-rowcount={5} aria-colcount={5}>{Array.from({ length: 25 }, (_, cell) => { const hit = moleHits.some((entry) => entry.cell === cell); const visible = moleVisible(cell); const mole = challenge.moles?.find((candidate) => candidate.cell === cell); return <button key={cell} ref={(button) => { moleButtons.current[cell] = button; }} className={hit ? styles.moleHit : visible ? styles.moleVisible : styles.mole} type="button" role="gridcell" tabIndex={cell === focusCell ? 0 : -1} aria-rowindex={Math.floor(cell / 5) + 1} aria-colindex={(cell % 5) + 1} aria-disabled={!visible || hit} aria-pressed={hit} aria-label={visible ? `Mole in cell ${cell + 1}` : `Empty cell ${cell + 1}`} onFocus={() => setFocusCell(cell)} onKeyDown={(event) => moveMoleFocus(event, cell)} onClick={() => { if (!visible || !mole || hit) return; void bridge.recordMoleHit(lockoutId, challenge.nonce, cell).then((result) => { if (result.ok) setMoleHits((current) => [...current, { id: mole.id, cell }]); else setNotice(result.code); }).catch((error: unknown) => setNotice(error instanceof Error ? error.message : String(error))); }}>{visible ? 'Mole' : ''}</button>; })}</div><button className={styles.button} type="button" disabled={!moleComplete} onClick={() => void submit({ kind: 'mole-round' })}>{moleComplete ? activeLabels?.submitMoles ?? 'Submit mole round' : activeLabels?.roundProgress ?? 'Round in progress'}</button></div> : null}
