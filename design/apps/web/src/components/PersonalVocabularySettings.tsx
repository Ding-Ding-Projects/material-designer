...
} from './universal/universalSettings';
...
interface PersonalVocabularySettingsProps {
  onHistoryMutation?: (
...
function useSchoolMode(): boolean {
  const read = useCallback(() => readUniversalSettings().school.enabled, []);
  const [enabled, setEnabled] = useState(read);
  useEffect(() => {
    const unsubscribe = subscribeToPersonalVocabularySchoolMode(setEnabled);
...
export function PersonalVocabularySettings({ onHistoryMutation }: PersonalVocabularySettingsProps) {
  const c = useCopy();
    ...
    return (
      <section className={styles.section} data-od-setting="personalVocabulary" aria-labelledby="personal-vocabulary-title">
        <div className={styles.header}>
          ...
          <RegexSearchField search={search} fieldLabel={privateCopy({ en: 'personal wording settings', yue: 'personal wording 設定' })} placeholder={privateCopy({ en: 'Search this surface', yue: '搜尋呢個 surface' })} testId="personal-vocabulary-search" />
        </div>
    ...
    <section className={styles.section} data-od-setting="personalVocabulary" aria-labelledby="personal-vocabulary-title">
      ...
      {showUpload ? <div className={styles.row} data-od-setting="personalVocabulary.upload">
        ...
        <label className={styles.fileButton}>
          ...
          <input key={fileInputKey} type="file" accept=".json,application/json" aria-label={privateCopy({ en: 'Choose a local vocabulary JSON file', yue: '揀本地 vocabulary JSON file' })} onChange={(event) => void onFile(event.target.files?.[0])} />
        </label>
      </div> : null}
