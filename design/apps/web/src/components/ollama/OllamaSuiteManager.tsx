...
export function OllamaSuiteManager() {
  ...
  const catalogRefreshIdRef = useRef('');
  ...
  const refresh = useCallback(async () => {
    ...
    const catalogResult = await collectCatalog((token, signal) => client.catalogPage(token, signal, selectedModelRef.current || null, catalogRefreshIdRef.current).then((result) => result.ok ? result.value : Promise.reject(new Error(result.error.message))), controller.signal);
    if (generation !== refreshGenerationRef.current) return;
    if (catalogResult.ok) {
