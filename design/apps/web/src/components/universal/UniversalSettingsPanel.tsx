...
function copy(key: string, state: UniversalSettingsState, vars: Record<string, string | number> = {}): string {
  ...
  const level = state.funnyEnglish;
  const levelItem = item.levels?.[level];
  const en = levelItem?.en ?? item.en;
  const yue = levelItem?.yue ?? item.yue;
...
function useUniversalSettings(): [UniversalSettingsState, (patch: Partial<UniversalSettingsState>) => void] {
  const [state, setState] = useState<UniversalSettingsState>(() =>
    getOpenDesignHost()?.universalSettings ? createDefaultUniversalSettings() : readUniversalSettings(),
  );
  const stateRef = useRef(state);
  const writeQueueRef = useRef<Promise<unknown>>(Promise.resolve());
  useEffect(() => {
    const bridge = getOpenDesignHost()?.universalSettings;
    if (bridge) {
      let mounted = true;
...
export function UniversalSettingsPanel({ appVersionInfo = null, initialSection = 'language' }: UniversalSettingsPanelProps) {
  ...
  const visibleSections = state.school.enabled ? SECTION_IDS.filter((id) => id === 'school' || id === 'status') : SECTION_IDS;
...
function SchoolSection({ state, update }: { state: UniversalSettingsState; update: (patch: Partial<UniversalSettingsState>) => void }) {
  ...
  return <SectionShell id="school" title={copy('school', state)} hint={copy('schoolHelp', state)} state={state} items={items}>
    <div className={styles.cardGrid}>
      ...
      {authOpen && authRevision !== null ? <ToyLockAuthenticationPopover targetId="general" targetLabel={state.school.name} policy={policy} anchor={schoolControlRef.current} verifyFactor={async (request: ToyLockVerificationRequest) => { const host = getOpenDesignHost(); if (!host?.toyLocks) return false; const factors = request.factor === 'pin' ? { pin: request.value } : request.factor === 'password' ? { password: request.value } : { totp: request.value }; const result = await host.toyLocks.verify({ targetId: 'general', policy: request.policy, revision: authRevision, factors }); return result.ok && result.matched; }} onAuthenticated={() => { setAuthOpen(false); update({ school: { ...state.school, enabled: false } }); }} onCancel={() => setAuthOpen(false)} /> : null}
    </div>
  </SectionShell>;
}
...
function StatusSection({ state, version, updatedAt, sourceRevision }: { state: UniversalSettingsState; version: string | null; updatedAt: string | null; sourceRevision: string | null }) {
  ...
  useEffect(() => {
    const hub = getOpenDesignHost()?.statusHub;
    if (!hub) {
