...
export const UNIVERSAL_SURFACE_SEARCH_INVENTORY = Object.freeze([
  ...
  'narrator-language', 'english-voice', 'cantonese-voice', 'schedule-source', 'notifications-list',
] as const);
...
export function validateScheduleRule(rule: UniversalScheduleRule): string | null {
  ...
  if (rule.source === 'api' && (!rule.sourceUrl || !/^https:\/\//.test(rule.sourceUrl))) return 'API schedules require an HTTPS URL.';
  if (rule.source === 'homeAssistant' && (!rule.sourceBaseUrl || !/^https:\/\//.test(rule.sourceBaseUrl) || !rule.sourceEntity)) return 'Home Assistant schedules require an HTTPS base URL and boolean entity.';
  return null;
}
...
export function scheduleRuleMatches(rule: UniversalScheduleRule, date: Date): boolean {
  ...
  if (rule.weekdays !== 'all' && !rule.weekdays.includes(day)) return false;
  ...
  if (rule.startTime && rule.endTime) {
    ...
    } else if (currentTime > rule.endTime && currentTime < rule.startTime) {
      return false;
    }
  }
  return true;
