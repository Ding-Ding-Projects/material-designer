...
function boundedString(value: unknown, pattern: RegExp = NAME_PATTERN): value is string {
  return typeof value === 'string' && value.length > 0 && value.length <= MAX_APPEARANCE_STRING && pattern.test(value);
}
...
function validateStyle(value: unknown, seen: WeakSet<object>): value is AppearanceStateStyle {
  ...
  return withNode(value, seen, () => Array.isArray(value.layers)
    ...
    && value.layers.length <= MAX_APPEARANCE_LAYERS
    && value.layers.every((layer) => validateLayer(layer, seen))
    && uniqueIds(value.layers)
    && Array.isArray(value.selections)
    && value.selections.length <= MAX_APPEARANCE_COLLECTION
    && value.selections.every((selection) => validateSelection(selection, seen))
    && uniqueIds(value.selections)
    && Array.isArray(value.channels)
    && value.channels.length > 0
    && value.channels.length <= MAX_APPEARANCE_COLLECTION
    && value.channels.every((channel) => boundedString(channel))
    && Array.isArray(value.masks)
    && value.masks.length <= MAX_APPEARANCE_COLLECTION
    && value.masks.every((mask) => boundedString(mask))
    && Array.isArray(value.channelState)
    && value.channelState.length > 0
    && value.channelState.length <= MAX_APPEARANCE_COLLECTION
    && value.channelState.every((channel) => validateChannel(channel, seen))
    && uniqueIds(value.channelState)
    && Array.isArray(value.maskState)
    && value.maskState.length <= MAX_APPEARANCE_COLLECTION
    && value.maskState.every((mask) => validateMask(mask, seen))
    && uniqueIds(value.maskState)
    && Array.isArray(value.adjustments)
    && value.adjustments.length <= MAX_APPEARANCE_COLLECTION
    && value.adjustments.every((adjustment) => validateAdjustment(adjustment, seen))
    && uniqueIds(value.adjustments)
    && Array.isArray(value.smartObjects)
    && value.smartObjects.length <= MAX_APPEARANCE_COLLECTION
    && value.smartObjects.every((smartObject) => validateSmartObject(smartObject, seen))
    && uniqueIds(value.smartObjects)
    ...
    && [1, 2, 3, 4, 5].includes(value.rainbowSpeedLevel)
    ...
    && value.channels.every((channel, index) => channel === value.channelState[index]?.id)
    && value.masks.length === value.maskState.length
    && value.masks.every((mask, index) => mask === value.maskState[index]?.id)) ?? false;
}
...
function validateAppearance(value: unknown, expectedTargetId?: string): value is ElementAppearance {
  ...
  return withNode(value, seen, () => boundedId(value.targetId)
    ...
    && exactKeys(value.states, APPEARANCE_STATES)
    && APPEARANCE_STATES.every((state) => validateStyle(value.states[state], seen))
    && validateInheritance(value.states as Record<AppearanceState, AppearanceStateStyle>)
...
function validExportObject(value: unknown): value is ElementAppearanceExport {
  if (!isPlainObject(value) || !exactKeys(value, EXPORT_KEYS)) return false;
  let serializedLength = Number.POSITIVE_INFINITY;
  try { serializedLength = JSON.stringify(value).length; } catch { return false; }
  return value.schema === APPEARANCE_SCHEMA
    && value.version === APPEARANCE_SCHEMA_VERSION
    && boundedId(value.targetId)
    && validateAppearance(value.appearance, value.targetId)
    && serializedLength <= MAX_APPEARANCE_EXPORT_BYTES;
}

export function parseElementAppearanceExport(value: unknown): ElementAppearanceExport | null {
  if (!validExportObject(value)) return null;
...
function normalizeLegacyAppearance(value: unknown, targetId: string): ElementAppearance | null {
  ...
  const stateRecord = isPlainObject(raw.states) ? raw.states : {};
...
function ensureLoaded(): void {
  ...
  history = Array.isArray(storedHistory)
    ? storedHistory.filter((item): item is AppearanceHistoryEntry => isPlainObject(item) && boundedId(item.targetId) && typeof item.id === 'string' && typeof item.action === 'string' && typeof item.at === 'string' && validateAppearance(item.snapshot, item.targetId)).slice(-MAX_APPEARANCE_HISTORY)
    : [];
}
...
export function setRainbowSpeedLevel(value: number): void {
  ...
  if (typeof window !== 'undefined') {
    ...
    document.documentElement.style.setProperty('--element-appearance-rainbow-duration', RAINBOW_DURATIONS[level - 1]);
    window.dispatchEvent(new CustomEvent('open-design:appearance-rainbow-speed', { detail: { level, duration: RAINBOW_DURATIONS[level - 1] } }));
  }
  notify();
}
...
export function applyAppearanceStateToElement(element: RenderedElement | null, state: AppearanceStateStyle, stateId: AppearanceState = 'normal'): void {
  ...
  style.textTransform = state.capitalization === 'none' ? 'none' : state.capitalization;
...
export function serializeElementAppearance(targetId: string): string {
  ...
  if (serialized.length > MAX_APPEARANCE_EXPORT_BYTES) throw new Error('Appearance export exceeds the bounded byte limit.');
  return serialized;
}
...
export function appearanceTargetPath(element: RenderedElement): string {
  ...
  if (dataId) return `[data-appearance-id="${CSS.escape(dataId)}"], [data-testid="${CSS.escape(dataId)}"]`;
...
export function useAppearanceRegistry(): AppearanceRegistry {
  ...
  const register = useCallback((target: AppearanceTarget) => {
    ...
    if (!previous || previous.element !== target.element || previous.label !== target.label || previous.role !== target.role || previous.path !== target.path) rerender((value) => value + 1);
