...
function preflightGraph(
  ...
  try {
    for (const key of Object.keys(value)) {
      const issue = preflightGraph(value[key], `${path}.${key}`, depth + 1, ancestors, state);
...
function boundedString(value: unknown, path: string, maximum = APPEARANCE_EXPORT_LIMITS.maxStringBytes): AppearanceExportValidationIssue | null {
