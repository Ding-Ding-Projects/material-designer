...
describe('packaged sidecar environment builders', () => {
  ...
  it('uses the bundled standalone web output for capture while preserving the isolated profile', () => {
    ...
    expect(env).toMatchObject({
      ...
      OD_WEB_STANDALONE_ROOT: join(
        '/tmp/od-pkg/resources',
        'open-design-web-standalone',
      ),
      HOME: join('/tmp/od-capture-run', 'home'),
      USERPROFILE: join('/tmp/od-capture-run', 'home'),
      TMPDIR: join('/tmp/od-capture-run', 'tmp'),
