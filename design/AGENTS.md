...
- A bilingual render interpolates **per language and then joins** — `renderBilingual` in `apps/web/src/i18n/interpolate.ts`, the pure half that `index.tsx` re-exports. An interpolation variable whose value is itself translated copy must therefore be passed as its KEY, wrapped in `tv(key, vars?, transform?)`, never as a `t()` result: a `t()` result is already bilingual, so the bilingual template substitutes it into both halves and the value is said twice (`{level} density` + `Default · 預設` gave `Default · 預設 density · Default · 預設密度`). `tv` resolves through the same per-language read that renders the template, so both funny-level sliders still apply to the nested value; `tForLanguageTag` does not and must not be used to work around this.
