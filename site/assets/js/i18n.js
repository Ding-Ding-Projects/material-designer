...
const CATALOGUE = {
  ...
  'st.r5.site': { en: '— strip, overflow, reordering, pinning, a searchable list and persistence. Grouping and bulk close are deliberately absent: the site has eleven permanent sections with nothing to group and nothing safe to close.', yue: '— 分頁條、溢出、重新排序、釘住、可搜尋清單同狀態保存。分組同批次關閉係特登冇：呢個網站有十一個永久區段，冇嘢好分組，亦冇一個關咗係安全嘅。' },
  ...
  'st.r7.site': { en: '— clearing the browser-local converter queue requires two independent keys, the exact affected count, a full-range slider, Escape, and an emergency exit.', yue: '— 清除瀏覽器本機轉換隊列之前，要操作兩把獨立鎖匙、睇清楚確實數量、拉完整條確認滑桿，亦可以用 Escape 或緊急離開取消。' },
