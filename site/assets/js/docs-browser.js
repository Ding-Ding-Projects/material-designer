...
function safeLocalImageUrl(value) {
  ...
  if (!raw || raw.startsWith('//') || /^([a-z]+:|data:)/i.test(raw)) return null;
  const normalized = normalisePath(raw);
...
function inlineMarkdown(value, article) {
  ...
  source = source.replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g, (whole, alt, target) => {
    const local = safeLocalImageUrl(target);
...
function markdownToHtml(markdown, article) {
  ...
  while (index < lines.length) {
    ...
    if (heading) {
      ...
      html.push('<h' + level + ' id="doc-' + escapeAttr(id) + '">' + inlineMarkdown(heading[2], article) + '</h' + level + '>');
      index += 1;
      continue;
    }
...
function ensureManifest(value) {
  ...
  for (const article of value.articles) {
    if (!article || typeof article.id !== 'string' || typeof article.path !== 'string' ||
        typeof article.title !== 'string' || typeof article.markdown !== 'string' || seen.has(article.id)) {
    ...
    if (path !== article.path || path.includes('..') || !path.endsWith('.md')) {
  ...
  function openArticle(article, anchor) {
    ...
    if (anchor) {
      requestAnimationFrame(() => {
        const target = document.getElementById('doc-' + anchor.toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-'));
