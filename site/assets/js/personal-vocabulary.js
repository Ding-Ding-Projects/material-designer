...
import {
  normalize as normalizeUniversalSettings,
...
export function validatePersonalVocabularyText(source) {
  ...
  for (const key of keys) {
    ...
    if (/\d/u.test(key)) return { ok: false, code: 'factual-key' };
...
function writeCache(payload) {
  ...
  } catch { restoreCache(previous); return { ok: false, code: 'storage-unavailable' }; }
}
...
function clearCache() {
  ...
  try {
    ...
    if (localStorage.getItem(PERSONAL_VOCABULARY_STORAGE_KEY) !== null) return { ok: false, code: 'readback-mismatch' };
...
export function isPersonalVocabularySuppressed() {
  try {
    ...
    return normalizeUniversalSettings(raw ? JSON.parse(raw) : null).school.enabled;
  } catch { return false; }
}

export function applyPersonalVocabulary(text, payload, boundary = 'private-ui') {
  if (!payload || boundary !== 'private-ui') return text;
  let output = text;
  for (const [ordinary, replacement] of Object.entries(payload.entries)) output = output.split(ordinary).join(replacement);
  return output;
}
...
export function initPersonalVocabulary() {
  const root = document.querySelector('[data-personal-vocabulary]');
  if (!root) return;
  ...
  const schoolMode = () => isPersonalVocabularySuppressed();
  ...
  window.addEventListener('storage', (event) => { if (event.key === PERSONAL_VOCABULARY_STORAGE_KEY || event.key === PERSONAL_VOCABULARY_HISTORY_KEY || event.key === PERSONAL_VOCABULARY_SCHOOL_MODE_KEY) { payload = readCache(); paint(); filter(); renderHistory(); } });
  document.addEventListener(PERSONAL_VOCABULARY_EVENT, () => { payload = readCache(); paint(); filter(); renderHistory(); refreshI18n(); });
  document.addEventListener(PERSONAL_VOCABULARY_SCHOOL_MODE_EVENT, () => { payload = readCache(); paint(); filter(); renderHistory(); refreshI18n(); });
  document.addEventListener('md-i18n-applied', () => { paint(); filter(); });
  paint();
  filter();
  renderHistory();
}

export default { initPersonalVocabulary, validatePersonalVocabularyText, applyPersonalVocabulary };
