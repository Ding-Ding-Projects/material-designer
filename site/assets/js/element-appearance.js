 ...
 * prefers explicit data-appearance-id or id values. Anonymous DOM nodes get a
 * deterministic semantic digest, while unresolved collisions are reported as
 * unsupported instead of receiving an ordinal identity.
 */
...
function targetId(element) {
  ...
  if (element.closest('[data-appearance-editor]')) return null;
...
function editor(target, close, regex, copy) {
  ...
  const render = () => {
    ...
    body.querySelector('[data-reset-property]')?.addEventListener('change', (event) => { const key = event.target.value; const defaults = styleDefault(); if (Object.prototype.hasOwnProperty.call(defaults, key)) { recordValue.states[active][key] = clone(defaults[key]); record(target, recordValue, `Reset ${key}`); render(); } });
  ...
  host.querySelector('[data-close]').addEventListener('click', () => { host.remove(); window.removeEventListener('resize', place); window.removeEventListener('scroll', place, true); close(); }); host.querySelector('[data-undo]').addEventListener('click', () => { const entries = history.filter((entry) => entry.targetId === target.id); const entry = entries[entries.length - 1]; if (!entry) return; const current = clone(records[target.id] || recordValue); const stack = redoRecords.get(target.id) || []; stack.push(current); redoRecords.set(target.id, stack); records[target.id] = clone(entry.snapshot); history.push({ id: `${Date.now()}-${target.id}-undo`, targetId: target.id, action: 'Undo appearance', at: now(), snapshot: current }); write(STORAGE_KEY, records); write(HISTORY_KEY, history.slice(-MAX_HISTORY)); apply(target, resolved(records[target.id], records[target.id].activeState), records[target.id].activeState); Object.assign(recordValue, clone(records[target.id])); render(); }); host.querySelector('[data-redo]').addEventListener('click', () => { const stack = redoRecords.get(target.id); const next = stack?.pop(); if (!next) return; const current = clone(records[target.id] || recordValue); history.push({ id: `${Date.now()}-${target.id}-redo`, targetId: target.id, action: 'Redo appearance', at: now(), snapshot: current }); records[target.id] = clone(next); write(STORAGE_KEY, records); write(HISTORY_KEY, history.slice(-MAX_HISTORY)); apply(target, resolved(next, next.activeState), next.activeState); Object.assign(recordValue, clone(next)); render(); }); host.querySelector('[data-export]').addEventListener('click', () => { const blob = new Blob([JSON.stringify({ schema: 'material-designer.element-appearance', version: 1, targetId: target.id, appearance: recordValue }, null, 2)], { type: 'application/json' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `${target.id.replace(/[^a-zA-Z0-9_-]/g, '_')}-appearance.json`; link.click(); URL.revokeObjectURL(link.href); });
  ...
  host.addEventListener('keydown', (event) => { if (event.key === 'Escape') { event.preventDefault(); host.querySelector('[data-close]').click(); } });
  render(); renderPresets(host, target, recordValue, active, render); document.body.append(host); window.addEventListener('resize', place); window.addEventListener('scroll', place, true); host.querySelector('[data-close]').focus();
}
