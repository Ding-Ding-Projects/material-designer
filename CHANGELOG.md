...
- **Shared menu callbacks now carry typed lifecycle receipts.** Appearance,
  locking, destructive confirmation, and locked-dropdown authentication all
  require target-bound `requested`, `opened`, `completed`, or `cancelled`
  results. Duplicate option, item, and owner ids
  ...
  **Shared menu callbacks 而家要 typed lifecycle receipts。** Appearance、lock、
  ...
  嘅 requested、opened、completed 或 cancelled result。Duplicate options、items 同 owners 會 fail closed，
