...
- **Settings toy-lock review repairs now authenticate existing-lock replacement and removal, connect tab appearance to an exact anchored per-tab editor, and state QR evidence limits honestly.** Existing credentials must pass the current revisioned policy before either mutator runs, while support-only routes remain unable to expose those controls. The application-level appearance callback opens a persisted editor for the supplied section and anchor, and restores focus when it closes. The bundled QR decoder and Reed-Solomon verifier remain useful structural evidence, but independent packaged scanner interoperability is explicitly unverified. Recovery-folder inventory, stable Support Tickets locators, and clipboard-unavailable corner notification feedback are covered by focused source contracts. Hosted checks and packaged interaction remain pending.
