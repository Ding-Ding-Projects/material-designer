...
- **Create approved evidence parents before writes and bind CDP to the exact committed page.** Commit [`ba381c8b190ad3803ed2f9631913d7421199850d`](https://github.com/Ding-Ding-Projects/material-designer/commit/ba381c8b190ad3803ed2f9631913d7421199850d) repairs the final two source-level evidence gaps. The live path now initializes exactly eight fixed no-reparse record parents, stages the selected artifact below `artifacts/<sha256>`, creates only the exact run-image parent, and re-resolves every evidence leaf immediately before writing. The live binding now carries `expectedPageUrl`; every CDP evaluation requires exact normalized equality with the sole target URL, and the actual URL plus digest are bound through transcript, live origin, capture run, receipt, static validation, and ledger. PowerShell 5.1 and PowerShell 7 pass an empty-root green fixture, five path/reparse reds, and 18 live-origin reds including fully rehashed same-origin wrong-path, foreign-URL, and redirected-URL records. No driver ran and the evidence root remains absent.

  **寫 evidence 之前先起 approved parent，CDP 亦要對中 committed page。** Commit [`ba381c8b190ad3803ed2f9631913d7421199850d`](https://github.com/Ding-Ding-Projects/material-designer/commit/ba381c8b190ad3803ed2f9631913d7421199850d) 修好最後兩個 source-level evidence 缺口。Live path 依家會先建立正正八個固定 no-reparse record parent，將 selected artifact 放入 `artifacts/<sha256>`，只建立 exact run-image parent，而且每次寫 evidence leaf 之前都重新 resolve。Live binding 新增 `expectedPageUrl`，每次 CDP evaluation 都要求唯一 target URL 經 normalization 後完全一致，actual URL 同 digest 會一路綁入 transcript、live origin、capture run、receipt、static validation 同 ledger。PowerShell 5.1 同 PowerShell 7 都通過 empty-root green fixture、5 個 path/reparse red，同 18 個 live-origin red，包括重新綁齊 hash 嘅 same-origin wrong path、foreign URL 同 redirected URL。今次冇叫 driver，evidence root 依然唔存在，門都未開就唔會扮已經入屋。

- **Serialize private live evidence writes before reading ledger state.** Commit [`5887fbdaa2a24c601a44204db99d31a272fd94bd`](https://github.com/Ding-Ding-Projects/material-designer/commit/5887fbdaa2a24c601a44204db99d31a272fd94bd) puts the Git-admin cross-process lock inside the unexported capability-checked writer. Two legitimate live sessions can no longer read the same next sequence before either has appended. The lock is released in `finally`, while atomic replacement, bounded sharing retry, rollback, and final reopen proof remain unchanged.

  **Private live evidence 寫入會先排隊再讀 ledger。** Commit [`5887fbdaa2a24c601a44204db99d31a272fd94bd`](https://github.com/Ding-Ding-Projects/material-designer/commit/5887fbdaa2a24c601a44204db99d31a272fd94bd) 將 Git-admin cross-process lock 放入 unexported capability-checked writer。兩個真 live session 唔會再一齊讀到同一個 next sequence，lock 會喺 `finally` 放返，atomic replacement、有限 sharing retry、rollback 同最後 reopen proof 全部照舊，門口終於有一條隊，唔係自由搏擊。

- **Require live driver origin before UI evidence can be promoted.** Commit [`867b3c75665db98c1c25894e27189b0c5b95b651`](https://github.com/Ding-Ding-Projects/material-designer/commit/867b3c75665db98c1c25894e27189b0c5b95b651) closes the remaining static-origin hole. Static receipts and transcripts now validate structure only, every nonempty static ledger reports structural-only, and the public append script always refuses. The sole production path mints a private in-process capability and nonce, derives the fixed cheap headless executable from the known Documents location, communicates through a bounded stdin bridge with a minimal environment, resolves the current process and one nonzero target window, delivers the committed input, collects bounded semantic polls and a fresh original PNG, validates helper, executable, transcript, action, process, window, time, replay, and nonce identities, writes through an unexported capability-checked function, and revokes the capability in `finally`. The committed live-driver registry and ledger remain empty. PowerShell 5.1 and PowerShell 7 passed 13 schema fixtures, 22 schema/admission negatives, 18 evidence negatives, 15 live-origin negatives, 5 privacy negatives, 3 reparse negatives, 6 observed sharing retries, and two serialized cross-process lock writes. The source-tested no-row production route invoked no driver, created no evidence root, and changed no ledger bytes.

  **UI evidence 要有真 live driver 出身先可以升級。** Commit [`867b3c75665db98c1c25894e27189b0c5b95b651`](https://github.com/Ding-Ding-Projects/material-designer/commit/867b3c75665db98c1c25894e27189b0c5b95b651) 封咗最後一個 static-origin 窿。Static receipt 同 transcript 依家只可以驗結構，任何 nonempty static ledger 都只會報 structural-only，public append script 直頭拒單。唯一 production path 會即場 mint private in-process capability 同 nonce，由 known Documents location 搵固定 cheap headless executable，用有限 stdin bridge 同 minimal environment 溝通，resolve 當前 process 同唯一非零 target window，送出 committed input，收有限 semantic poll 同新鮮原圖 PNG，再逐樣驗 helper、executable、transcript、action、process、window、時間、replay 同 nonce identity，最後只畀 unexported capability-checked function 寫入，仲會喺 `finally` 撤銷 capability。Committed live-driver registry 同 ledger 依然係空。PowerShell 5.1 同 PowerShell 7 都通過 13 個 schema fixture、22 個 schema/admission negative、18 個 evidence negative、15 個 live-origin negative、5 個 privacy negative、3 個 reparse negative、6 次實際 sharing retry，同兩個 serialized cross-process lock write。Source-tested no-row production route 冇叫 driver、冇建立 evidence root、冇改任何 ledger byte，紙紮跑車今次終於攞唔到行車證。

- **Harden UI-drive evidence so a convincing JSON costume cannot pass as runtime proof.** Commit [`c317c49f41b487fb7fede3f05101689b6f1950a8`](https://github.com/Ding-Ding-Projects/material-designer/commit/c317c49f41b487fb7fede3f05101689b6f1950a8) adds duplicate-preserving strict JSON admission and executes draft-2020-12 schemas for the complete evidence chain. A separate digest-bound authority fixes 30 features, two surfaces, ten destinations, and 70 scenes. Version 3 receipts now resolve real Git commits, require exact source and built-from equality, bind artifact provenance, an approved-driver capture run, the original bounded PNG, every-element audit, and one fixed six-kind output manifest, then compare every tuple and action field including ordered `allowedOrigins`. Version 2 ledger rows retain every identity plus receipt and manifest hashes. Appends use a cross-process lock, bounded Windows sharing retries, atomic replacement, rollback, and final reopen and full-chain revalidation. Both PowerShell 5.1 and PowerShell 7 passed 9 valid schema fixtures, 21 strict/schema/bound negatives, 18 evidence negatives, 5 privacy negatives, 3 reparse negatives, and a two-process append race that observed 6 sharing retries. The committed ledger remains empty and no build, launch, UI interaction, capture, or real evidence promotion occurred.

  **UI-drive evidence 今次唔再畀著住 JSON 西裝嘅假證據扮 runtime proof。** Commit [`c317c49f41b487fb7fede3f05101689b6f1950a8`](https://github.com/Ding-Ding-Projects/material-designer/commit/c317c49f41b487fb7fede3f05101689b6f1950a8) 加咗 duplicate-preserving strict JSON admission，亦真係執行 draft-2020-12 schema，唔係淨係擺個 `$schema` 招牌。獨立 digest-bound authority 鎖實 30 個 feature、兩個 surface、十個 destination 同 70 個 scene。Version 3 receipt 要 resolve 真 Git commit、source 同 built-from 完全一致，再綁 artifact provenance、approved-driver capture run、有限原圖 PNG、every-element audit，同唯一一份六類 output manifest，連 ordered `allowedOrigins` 在內每個 tuple 同 action field 都逐個對。Version 2 ledger 留齊 identity、receipt hash 同 manifest hash；append 有 cross-process lock、有限 Windows sharing retry、atomic replacement、rollback，同最後 reopen 加完整 chain 重驗。PowerShell 5.1 同 PowerShell 7 都通過 9 個 valid schema fixture、21 個 strict/schema/bound negative、18 個 evidence negative、5 個 privacy negative、3 個 reparse negative，同一個真兩 process append race，仲實際見到 6 次 sharing retry。Committed ledger 依然係空，今次冇 build、launch、UI interaction、capture 或 real evidence promotion，冇證據就唔扮有，個 validator 終於唔再靠西裝領呔辦案。
