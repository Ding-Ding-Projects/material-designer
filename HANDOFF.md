...
case), notification checks pass (3 cases), and six lock/QR failures reproduce
against unchanged source. Independent review accepted the corrected dock.
Production diagnostics were reduced to coordinated source changes now incorporated
plus sidecar declarations generated by the build. The final build remains unrun.

The existing Release workflow will be invoked manually for priority publication.
Its unrelated documentation/screen-recording verification step is removed from
Actions; these checks remain available locally. The build consumes the committed
documentation bundle. The unfinished separate manual publisher is retained outside
main and will not be used for this release.
