...
- The self-hosted workflows now run committed, user-scoped bootstrap scripts for pinned `gh`, `jq` and `7z` when those tools are absent. The scripts passed Bash syntax, PowerShell AST parsing and `git diff --check`; no Node, pnpm or Electron command was run locally. Labelled-runner execution remains unverified.
