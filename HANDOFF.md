...
> [!NOTE]
> **Converter host repair, source-only, 2026-08-27.** Commit
> [`aac585db`](https://github.com/Ding-Ding-Projects/material-designer/commit/aac585db256cf7295a517e375f444e9f85ef0e1d)
> adds fixed-size on-disk queue indexing, bounded page reads, streaming journal
> compaction, incremental source-byte progress, one-use destination replacement
> authorization, exclusive promotion with rollback, durable host notifications,
> and redacted local Git history records. The renderer mounts the two-key
> full-range overwrite confirmation and persistent searchable dropdowns, while
> the browser converter keeps its explicit local-only boundary. Source tests and
> comment-aware red-green source checks are present, but this lane was not
> built, installed, or driven locally. Packaged runtime interaction, per-click
> captures, and hosted verdicts remain open.

> **Follow-up source repair:** commit
> [`271b3da5`](https://github.com/Ding-Ding-Projects/material-designer/commit/271b3da5a606ce28b4f274f0eb4194516691c060)
> streams legacy queue migration, removes the queue-length de-duplication set,
> and adds real changed-destination and concurrent-promotion tests. It remains
> local to `yum-tong-file-converter-20260827`; no push was performed.

> **Audit validation follow-up:** commit
> [`a3c1152c`](https://github.com/Ding-Ding-Projects/material-designer/commit/a3c1152c017263611536465f720b102ed9c0baa7)
> validates persisted notification and history records before the renderer
> receives them. The source branch remains local and has not been pushed.

**廣東話摘要：** `aac585db` 將 converter queue 改成 fixed-size on-disk index，逐頁
讀、逐筆 compaction，source bytes 逐段報 progress；已有 destination 由
one-use authorization、exclusive promotion 同 rollback 管理。Renderer 有兩把匙
加完整 slider，dropdown 自己保存 search 同 regex。今次係 source-only，未做
local build、install、runtime drive、capturing 或 hosted verdict。
