...
### 2026-08-29 - Record the reviewed Groups A through E source integration

**Reason:** The integrated source from base `77e67f23677156bc30b7059eea42faf8cb50e6d3` through
the current commit completes the reviewed Groups A through E source work. Group A repairs
deterministic parity route resolution and readiness witnesses while keeping Studio fail-closed.
Group B aligns shell chrome, overlays, portals, hit surfaces, and geometry tests. Group C repairs
settings and routine semantics, localized wrapping, touch targets, and focused source fixtures.
Group D migrates the reviewed shape and motion consumers onto the Material Design token contract,
keeps compatibility values explicit, and makes the CSS literal ledger exact. Group E repairs
declared-path handling for upstream-only files and adds its focused regression. Two touched
planning documents were restored to upstream-identical bytes and therefore are not listed below.
...
1. Deterministic parity must resolve only the exact owned route and tuple, so a missing Studio
   destination cannot be promoted by a lookalike page.
2. Shell chrome and overlay offsets must share one measured token contract, otherwise a surface
   can be present in source while its hit area sits under title-bar controls.
3. Localized settings and routine controls must retain their semantic names, states, wrapping,
   focus paths, and target sizes instead of relying on short English labels.
4. Shape and motion literals must be tracked by an exact ledger, with compatibility values named
   rather than silently changing timing or geometry during migration.
5. Declared-path checks must include upstream-only files, because an intentional deletion is still
   a real port difference and must not become a stale-notice false positive.
