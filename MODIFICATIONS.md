...
the current product tree exactly once. It adds the later daemon, runtime,
analytics, workflow-source, documentation, and interface foundations used by
the preserved feature histories. The current product source pin and manifest
remain authoritative, so every resulting difference is declared explicitly
rather than changing the baseline to hide the delta.
