...
// This is the executable source of truth for the shell check and the focused
...
// Source-only integration lanes may reuse an already-installed reviewed
// toolchain without creating a checkout-local dependency tree. The default
// remains the repository installation used by hosted verification.
const typescriptPath = process.env.REGEX_INVENTORY_TYPESCRIPT_PATH
  ? resolve(process.env.REGEX_INVENTORY_TYPESCRIPT_PATH)
  : resolve(root, 'design/node_modules/typescript/lib/typescript.js');
...
function htmlMarkerPresent(source, marker) {
  ...
  // looking at a tag so a commented example can never satisfy the check.
