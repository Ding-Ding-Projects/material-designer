...
const inventorySchema = readStrictJson(join(root, '.codex/verification/design-parity/inventory.schema.json'));
const inventoryDocument = readStrictJson(join(root, '.codex/verification/design-parity/inventory.json'));
assert.deepEqual(validateJsonSchema(inventoryDocument, inventorySchema), inventoryDocument);
const tupleDefaultsSchema = structuredClone(inventorySchema);
tupleDefaultsSchema.properties.defaults.$ref = '#/$defs/tuple';
assert.throws(() => validateJsonSchema(inventoryDocument, tupleDefaultsSchema), (error) => error.code === 'schema.required');
...
process.stdout.write(JSON.stringify({ ok: true, rows: 10, presentations: 6, pinnedInputs: 8, strictJsonNegatives: 6, schemaNegatives: 7, manifestSchemaPositive: true, reparseNegative: 1, sourceBoundaryNegatives: 2 }) + '\n');
