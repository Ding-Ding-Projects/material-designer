...
Assert-CodeLine $windowsWriterTestLines 'forced-kill-backup-intent-interval' 'The real native Chut must kill between rollback mutation and completion receipt.'
Assert-CodeLine $windowsWriterTestLines 'forced-kill-promotion-intent-interval' 'The real native Chut must kill between promotion mutation and completion receipt.'
Assert-CodeLine $windowsWriterTestLines 'initial-disposition-recovery' 'The real native Chut must recover an initial delete-pending and cleanup refusal.'
Assert-CodeLine $windowsWriterTestLines 'forced-kill-after-create-before-intent' 'The real native Chut must kill immediately after FILE_CREATE and before a temp identity receipt.'
Assert-CodeLine $windowsWriterTestLines 'metadata-clone\.tmp' 'The native Chut must install a cloned same-name substitute.'
Assert-CodeLine $windowsWriterTestLines 'Set-Acl' 'The native Chut must copy the original ACL onto the substitute.'
Assert-CodeLine $windowsWriterTestLines 'fsutil file queryEA' 'The native Chut must inspect copyable extended attributes.'
Assert-CodeLine $windowsWriterTestLines 'Final promoted output retained a recovery EA marker' 'The native Chut must prove final output carries no recovery EA.'
Assert-CodeLine $windowsWriterTestLines 'Move-Item\s+-LiteralPath\s+\$preIntentClone\s+-Destination\s+\$preIntentOriginal' 'The native Chut must install the metadata clone at the exact original basename.'
Assert-CodeLine $windowsWriterTestLines '\$preIntentGuardian\.Process\.Kill\(\)' 'The native Chut must kill the guardian itself before any identity receipt.'
Assert-CodeLine $windowsWriterTestLines 'handoff-clone-before-worker-guarded' 'The native Chut must attack the attempted early-release interval.'
Assert-CodeLine $windowsWriterTestLines '-not\s+\$handoffGuardian\.Process\.HasExited' 'The native Chut must prove guardian release cannot occur before worker-guarded.'
Assert-CodeLine $windowsWriterTestLines 'guardian-clear-before-ready-kill' 'The native Chut must kill after disposition clear and before guardian-ready.'
Assert-CodeLine $windowsWriterTestLines 'Recovery-ById' 'The native Chut must repeat exact file-ID provisional cleanup.'
...
Assert-CodeLine $auditLines '^\s*if\s*\(Buffer\.byteLength\(line,\s*"utf8"\)\s*>\s*MAX_ORDER_LINE_BYTES\)' 'Audit paging must bound each complete order record after splitting stream chunks.'
...
Assert-CodeLine $rendererLines '!preview\s*\|\|\s*preview\.adapterId\s*!==\s*adapter\.id\s*\|\|\s*preview\.targetFormat\s*!==\s*target' 'Queue and direct actions must require the exact current preview.'
Assert-CodeLine $rendererLines 'disclosureAcknowledgement\.previewId\s*!==\s*preview\.previewId' 'Loss disclosure acknowledgement must stay bound to the preview being consumed.'
...
if ($siteIntegrationAvailable) {
  ...
  Assert-True ($siteScript.Contains('indexedDB.open') -and $siteScript.Contains("openCursor(null, 'prev')") -and $siteScript.Contains('QUEUE_PAGE_SIZE')) 'Site queue must use bounded IndexedDB pages.'
  Assert-True (-not $siteScript.Contains('MAX_QUEUE_RECORDS') -and -not $siteScript.Contains('slice(-MAX_QUEUE_RECORDS)')) 'Site queue must not cap or silently discard older records.'
  Assert-True ($siteScript.Contains('cursor.advance(offset)') -and $siteHtml.Contains('data-converter-queue-previous') -and $siteHtml.Contains('data-converter-queue-next')) 'Site queue must expose bounded continuation paging.'
  Assert-True ($siteHtml.Contains('data-converter-clear-key="one"') -and $siteHtml.Contains('data-converter-clear-key="two"') -and $siteHtml.Contains('data-converter-clear-slider') -and $siteHtml.Contains('data-converter-clear-cancel')) 'Site queue clear must retain the two-key full-slider confirmation and emergency exit.'
  Assert-True ($siteScript.Contains('function readQueueOffset()') -and $siteScript.Contains('function writeQueueOffset(offset)') -and $siteScript.Contains('catch {')) 'Site queue paging preferences must fail safely when browser storage is unavailable.'
} else {
...
Assert-RedMutation $windowsWriterTest "  `$preIntentKillRoot = Join-Path `$caseRoot 'forced-kill-after-create-before-intent'" "  `$preIntentKillRootRemoved = Join-Path `$caseRoot 'removed'" 'forced-kill-after-create-before-intent' 'the native Chut must retain the pre-identity FILE_CREATE kill'
...
Assert-RedMutation $audit '      if (Buffer.byteLength(line, "utf8") > MAX_ORDER_LINE_BYTES) {' '      if (false) {' '^\s*if\s*\(Buffer\.byteLength\(line,\s*"utf8"\)\s*>\s*MAX_ORDER_LINE_BYTES\)' 'audit paging must bound complete lines after splitting stream chunks'
...
Assert-RedMutation $renderer "    if (!preview || preview.adapterId !== adapter.id || preview.targetFormat !== target) { setMessage(copy('previewRequired')); return; }" "    if (!preview) { setMessage(copy('previewRequired')); return; }" '!preview\s*\|\|\s*preview\.adapterId\s*!==\s*adapter\.id\s*\|\|\s*preview\.targetFormat\s*!==\s*target' 'queue and direct actions must consume the exact current preview'
...
Assert-RedMutation $siteScript 'const QUEUE_PAGE_SIZE = 50;' 'const QUEUE_PAGE_SIZE_REMOVED = 50;' '^const\s+QUEUE_PAGE_SIZE\s*=\s*50;' 'the site queue must retain bounded page reads'
Assert-RedMutation $siteScript '          cursor.advance(offset);' '          cursor.continue();' '^\s*cursor\.advance\(offset\);' 'the site queue must retain bounded continuation paging'
Assert-RedMutation $siteHtml '      <input id="converter-clear-slider" type="range" min="0" max="100" value="0" disabled data-converter-clear-slider aria-describedby="converter-clear-gate-status">' '      <div data-converter-clear-slider></div>' 'data-converter-clear-slider.*type="range"|type="range".*data-converter-clear-slider' 'the site queue clear must retain its full-range confirmation slider'
Assert-RedMutation $siteScript 'function readQueueOffset() {' 'function readQueueOffset_removed() {' '^function\s+readQueueOffset\(\)\s*\{' 'site queue paging must retain a storage-refusal fallback'
if ($runtime.Contains('od:converter:queue:page')) {
  Assert-True (-not $runtime.Contains('queuedPreviewBindings')) 'Queued converter records must not retain one disclosure capability per item.'
  Assert-CodeLine $runtimeLines 'Lossy conversions require just-in-time review and cannot be added to the durable queue' 'Lossy queue work must fail honestly before a capability is retained.'
