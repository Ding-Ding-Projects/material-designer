...
Assert-CodeLine $nativeWriterLines '^\s*if\s*\(recovery_capability\s*!=\s*nullptr\)\s*ea_buffer\s*=\s*RecoveryEaInformation' 'Temporary creation must carry host recovery authority in the atomic NtCreateFile call.'
Assert-CodeLine $nativeWriterLines '^\s*&&\s*names\.has_capability\s*&&\s*VerifyRecoveryCapability\(remnant\.get\(\),\s*names\.capability' 'Pre-identity recovery must authenticate the exact creation capability.'
Assert-CodeLine $nativeWriterLines 'ClearRecoveryCapability\(temporary\.get\(\),\s*write_capability' 'The temporary capability must be removed before durable promotion.'
...
Assert-CodeLine $windowsWriterLines '^\s*const\s+recoveryCapability\s*=\s*randomBytes\(32\)\.toString\("hex"\);' 'The host must own recovery authority before starting the helper.'
Assert-CodeLine $windowsWriterLines '^\s*async\s+\#recoverCapability\(' 'The host must recover a pre-identity temporary without guessing its object identity.'
$writeAtomicStart = $windowsWriter.IndexOf('async writeAtomic(')
Assert-True ($windowsWriter.IndexOf('const recoveryCapability = randomBytes(32)', $writeAtomicStart) -lt $windowsWriter.IndexOf('const { child, reader } = await this.#start()', $writeAtomicStart)) 'Host recovery authority must exist before helper launch.'
...
Assert-RedMutation $nativeWriter '  if (recovery_capability != nullptr) ea_buffer = RecoveryEaInformation(*recovery_capability, false);' '  if (recovery_capability != nullptr) ea_buffer.clear();' '^\s*if\s*\(recovery_capability\s*!=\s*nullptr\)\s*ea_buffer\s*=\s*RecoveryEaInformation' 'creation must atomically carry host recovery authority'
Assert-RedMutation $nativeWriter '    && names.has_capability && VerifyRecoveryCapability(remnant.get(), names.capability, error);' '    && names.has_capability;' '^\s*&&\s*names\.has_capability\s*&&\s*VerifyRecoveryCapability\(remnant\.get\(\),\s*names\.capability' 'pre-identity recovery must verify the creation capability'
Assert-RedMutation $nativeWriter '  if (!ClearRecoveryCapability(temporary.get(), write_capability, &error)) {' '  if (false) {' 'ClearRecoveryCapability\(temporary\.get\(\),\s*write_capability' 'durable promotion must clear the private recovery capability'
...
Assert-RedMutation $windowsWriter '  async #recoverCapability(' '  async recoverCapability_removed(' '^\s*async\s+\#recoverCapability\(' 'the host must retain pre-identity capability recovery'
Assert-RedMutation $windowsWriter '    const recoveryCapability = randomBytes(32).toString("hex");' '    const recoveryCapability = "basename-only";' '^\s*const\s+recoveryCapability\s*=\s*randomBytes\(32\)\.toString\("hex"\);' 'the host must create cryptographic recovery authority before helper launch'
