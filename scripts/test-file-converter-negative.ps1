...
Assert-CodeLine $nativeWriterLines 'OpenChildById\(parent_path,\s*prepared_names\.temporary_witness' 'The worker must accept guardian authority through the non-copyable file ID.'
Assert-CodeLine $nativeWriterLines 'ParseNativeIdentity\(encoded\.substr\(second\s*\+\s*1\),\s*&names->temporary_witness\)' 'Prepared writer input must bind the guardian file ID.'
...
Assert-RedMutation $nativeWriter '  UniqueHandle temporary = OpenChildById(parent_path, prepared_names.temporary_witness, &error);' '  UniqueHandle temporary{};' 'OpenChildById\(parent_path,\s*prepared_names\.temporary_witness' 'worker handoff must use the non-copyable file ID'
Assert-RedMutation $nativeWriter '    && ParseNativeIdentity(encoded.substr(second + 1), &names->temporary_witness);' '    && true;' 'ParseNativeIdentity\(encoded\.substr\(second\s*\+\s*1\),\s*&names->temporary_witness\)' 'prepared worker input must retain the guardian file ID'
