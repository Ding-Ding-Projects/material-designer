...
function Write-UILiveEvidenceRecord {
    ...
    $transcriptPath=Resolve-UIEvidencePath -EvidenceRoot $root -Path $LiveCapture.TranscriptRelative -AllowMissingLeaf
    Write-UILiveJsonCreate $transcriptPath $LiveCapture.Transcript
    ...
    $origin=[ordered]@{version=1;originId=$LiveCapture.OriginId;originMode='live-private-in-process-capability';runId=$LiveCapture.RunId;sessionId=$LiveCapture.SessionId;sourceCommit=$LiveCapture.SourceCommit;artifactSha256=$LiveCapture.ArtifactSha256;orchestratorPath='scripts/run-approved-ui-drive-live.ps1';orchestratorSha256=$LiveCapture.OrchestratorSha256;modulePath='scripts/ui-drive-live-origin.psm1';moduleSha256=$LiveCapture.ModuleSha256;bridgePath='scripts/ui-drive-lowlevel-stdin-bridge.ps1';bridgeSha256=$LiveCapture.BridgeSha256;driverExecutablePathDigest=$LiveCapture.DriverExecutablePathDigest;driverExecutableSha256=$LiveCapture.DriverExecutableSha256;transcriptPath=$LiveCapture.TranscriptRelative;transcriptSha256=$transcriptHash;transcriptId=$LiveCapture.Transcript.transcriptId;nonceDigest=$LiveCapture.TranscriptNonceDigest;capabilityIdentityDigest=$LiveCapture.CapabilityIdentityDigest;processId=$LiveCapture.ProcessId;processImageSha256=$LiveCapture.ProcessImageSha256;windowClass=$LiveCapture.WindowClass;windowTitle=$LiveCapture.WindowTitle;windowWidth=$LiveCapture.WindowWidth;windowHeight=$LiveCapture.WindowHeight;sceneId=$LiveCapture.SceneId;interactionId=$LiveCapture.InteractionId;actionKind=$LiveCapture.ActionKind;actionTarget=$LiveCapture.ActionTarget;inputMethod=$LiveCapture.InputMethod;semanticPolls=@($LiveCapture.SemanticPolls);imagePath=$LiveCapture.ImagePath;imageSha256=$LiveCapture.ImageSha256;imageLastWriteUtc=$imageItem.LastWriteTimeUtc.ToString('o');startedAtUtc=$LiveCapture.StartedAtUtc.ToString('o');completedAtUtc=$LiveCapture.CompletedAtUtc.ToString('o');replayKey=Get-UILiveDigest ($LiveCapture.SessionId+'|'+$LiveCapture.RunId+'|'+$LiveCapture.ImageSha256+'|'+$LiveCapture.TranscriptNonceDigest)}
    $originPath=Resolve-UIEvidencePath -EvidenceRoot $root -Path $LiveCapture.OriginRelative -AllowMissingLeaf;Write-UILiveJsonCreate $originPath $origin;[void](Read-UIValidatedJson -Path $originPath -SchemaPath (Join-Path $schemaRoot 'live-origin.schema.json'));$originHash=Get-UIFileSha256 $originPath
    foreach($record in @($LiveCapture.SupportingRecords)){Write-UILiveJsonCreate $record.FullPath $record.Data;[void](Read-UIValidatedJson -Path $record.FullPath -SchemaPath $record.SchemaPath)}
    $receipt=$LiveCapture.ReceiptFactory.Invoke($originHash,$transcriptHash);$receiptPath=Resolve-UIEvidencePath -EvidenceRoot $root -Path $LiveCapture.ReceiptRelative -AllowMissingLeaf;Write-UILiveJsonCreate $receiptPath $receipt;[void](Read-UIValidatedJson -Path $receiptPath -SchemaPath (Join-Path $schemaRoot 'click-receipt.schema.json'));$receiptHash=Get-UIFileSha256 $receiptPath
    $manifest=$LiveCapture.ManifestFactory.Invoke($receiptHash,$originHash,$transcriptHash);$manifestPath=Resolve-UIEvidencePath -EvidenceRoot $root -Path $LiveCapture.ManifestRelative -AllowMissingLeaf;Write-UILiveJsonCreate $manifestPath $manifest;[void](Read-UIValidatedJson -Path $manifestPath -SchemaPath (Join-Path $schemaRoot 'approved-output-manifest.schema.json'));$manifestHash=Get-UIFileSha256 $manifestPath
...
function Invoke-UIApprovedLiveCapture {
    ...
    $artifact=Assert-UIPathHasNoReparsePoint -Path $ArtifactPath
    $artifactHash=Get-UIFileSha256 $artifact
    ...
    try{
        ...
        $before=Invoke-UICdpExpression -Port $port -Expression ([string]$binding[0].beforeExpression)
        ...
        for($index=1;$index -le [int]$binding[0].maxPollAttempts;$index++){$after=Invoke-UICdpExpression -Port $port -Expression ([string]$binding[0].afterExpression);$polls.Add([pscustomobject]@{ordinal=$index;elapsedMs=[int](([DateTime]::UtcNow-$started).TotalMilliseconds);observedState=$after;responseSha256=Get-UILiveDigest $after});if($after -ceq [string]$scene[0].expectedAfter){break};Start-Sleep -Milliseconds ([int]$binding[0].pollIntervalMs)}
        if($after -cne [string]$scene[0].expectedAfter){throw 'Bounded live semantic polls did not reach the expected state.'}
        $auditJson=Invoke-UICdpExpression -Port $port -Expression ([string]$binding[0].auditElementsExpression)
        ...
        $schemaRoot=Join-Path $RepositoryRoot '.codex/verification/ui-drive';$evidenceRoot=Join-Path $RepositoryRoot '.codex/verification/evidence'
        if(-not(Test-Path -LiteralPath $evidenceRoot)){New-Item -ItemType Directory -Path $evidenceRoot|Out-Null};[void](Assert-UIPathHasNoReparsePoint -Path $evidenceRoot)
        ...
        $imageRelative='images/{0}/{1:D4}-{2}.png' -f $runId,$sequence,$SceneId;$imageFull=Resolve-UIEvidencePath -EvidenceRoot $evidenceRoot -Path $imageRelative -AllowMissingLeaf;$imageParent=[IO.Path]::GetDirectoryName($imageFull);New-Item -ItemType Directory -Path $imageParent -Force|Out-Null
        ...
        $transcript=[ordered]@{version=1;transcriptId=$transcriptId;runId=$runId;sessionId=$sessionId;bridgePath='scripts/ui-drive-lowlevel-stdin-bridge.ps1';bridgeSha256=Get-UIFileSha256 $bridgePath;driverExecutablePathDigest=[string]$launch.driverExecutablePathDigest;driverExecutableSha256=[string]$launch.driverExecutableSha256;calls=$transcriptCalls}
        ...
        $support=@([pscustomobject]@{FullPath=Resolve-UIEvidencePath -EvidenceRoot $evidenceRoot -Path $provenanceRelative -AllowMissingLeaf;Data=$provenance;SchemaPath=Join-Path $schemaRoot 'artifact-provenance.schema.json'},[pscustomobject]@{FullPath=Resolve-UIEvidencePath -EvidenceRoot $evidenceRoot -Path $runRelative -AllowMissingLeaf;Data=$captureRun;SchemaPath=Join-Path $schemaRoot 'capture-run.schema.json'},[pscustomobject]@{FullPath=Resolve-UIEvidencePath -EvidenceRoot $evidenceRoot -Path $auditRelative -AllowMissingLeaf;Data=$audit;SchemaPath=Join-Path $schemaRoot 'every-element-audit.schema.json'})
