...
function Validate-Sources([hashtable]$source) {
  ...
  Require-Exact $source.windowsVault '^\s*if \(!verified \|\| !equal\(created, verified\)\) unavailable\(''Credential-vault key creation could not be verified\.''\);\s*$' 'vault key creation readback boundary'
  ...
  Require-Exact $source.windowsVaultTests '^\s*test\(''rejects a master-key readback mismatch instead of retaining a locally generated fallback'',.*$' 'credential-vault no-fallback test'
