...
$files = [ordered]@{
  ...
  windowsVault = Join-Path $root 'design/apps/desktop/src/main/authenticator/windows-credential-vault.ts'
  ...
  windowsVaultTests = Join-Path $root 'design/apps/desktop/tests/main/windows-credential-vault.test.ts'
...
function Validate-Sources([hashtable]$source) {
  ...
  Require-Exact $source.vault '^\s*export class CredentialVaultUnavailableError extends Error\s' 'typed credential-vault unavailable result'
  Require-Exact $source.vault '^\s*export interface OperatingSystemCredentialVault\s' 'injected credential-vault abstraction'
  Require-Exact $source.windowsVault '^\s*export class CredentialManagerBackend\s' 'Credential Manager backend'
  Require-Exact $source.windowsVault '^\s*export class WindowsCredentialVault implements OperatingSystemCredentialVault\s' 'Windows credential vault implementation'
  Require-Exact $source.windowsVault '^\s*export function createWindowsCredentialVault\(options: WindowsCredentialVaultOptions = \{\}\): OperatingSystemCredentialVault \{\s*$' 'credential vault factory'
  Require-Exact $source.windowsVault '^\s*export async function migrateLegacyElectronVault\(options: \{\s*$' 'explicit legacy vault migration seam'
  Require-Exact $source.windowsVault '^\s*\[DllImport\("Advapi32\.dll", CharSet=CharSet\.Unicode, SetLastError=true\)\] public static extern bool CredWrite\(ref CREDENTIAL credential, UInt32 flags\);\s*$' 'Credential Manager write boundary'
  Require-Exact $source.windowsVault '^\s*\[DllImport\("Advapi32\.dll", CharSet=CharSet\.Unicode, SetLastError=true\)\] public static extern bool CredRead\(string target, UInt32 type, UInt32 flags, out IntPtr credential\);\s*$' 'Credential Manager read boundary'
  Require-Exact $source.windowsVault '^\s*async ensureMaster\(name: string, candidate: Uint8Array\): Promise<Uint8Array> \{\s*$' 'atomic master-key backend contract'
  Require-Exact $source.windowsVault '^\s*const mutex = `Local\\\\MaterialDesignerCredentialVault_\$\{createHash\(''sha256''\)\.update\(qualified, ''utf8''\)\.digest\(''hex''\)\}`;\s*$' 'per-user namespaced native master-key mutex'
  Require-Exact $source.windowsVault '^\s*this\.\#masterKeyPromise \?\?= this\.\#createOrReadMasterKey\(\);\s*$' 'same-instance master-key singleflight'
  Require-Exact $source.windowsVault '^\s*if \(!verified \|\| !equal\(secret, verified\)\) unavailable\(''Legacy credential migration could not verify the target vault\.''\);\s*$' 'legacy migration readback boundary'
  Require-Exact $source.windowsVault '^\s*const timer = setTimeout\(\(\) => \{ child\.kill\(\); fail\(\); \}, 10_000\);\s*$' 'bounded Credential Manager helper deadline'
  Require-Exact $source.windowsVault '^\s*function decodeCanonicalBase64\(value: string, maximum: number\): Uint8Array \{\s*$' 'canonical bounded credential decoder'
  Require-Exact $source.windowsVault '^\s*const child = spawn\(executable, \[''-NoProfile'', ''-NonInteractive'', ''-ExecutionPolicy'', ''Bypass'', ''-Command'', credentialManagerScript\], \{ windowsHide: true, stdio: \[''pipe'', ''pipe'', ''ignore''\] \}\);\s*$' 'absolute PowerShell helper invocation'
  Require-Exact $source.windowsVault '^\s*inputStream\.once\(''error'', \(\) => \{ clearTimeout\(timer\); child\.kill\(\); fail\(\); \}\);\s*$' 'helper stdin failure handling'
  Require-Exact $source.windowsVault 'CredentialBlobSize -lt 1 -or \$credential\.CredentialBlobSize -gt 512\) \{ throw ''Credential Manager read size invalid'' \}' 'native Credential Manager read bound'
  Require-Exact $source.windowsVault '^\s*const keys = \[\.\.\.new Set\(options\.keys\)\]; if \(keys\.length > MAX_MIGRATION_KEYS\) unavailable\(''Legacy credential migration exceeds the bounded entry count\.''\);\s*$' 'bounded legacy migration inventory'
  Require-Exact $source.windowsVault '^\s*if \(options\.keys\.length > MAX_MIGRATION_KEYS\) unavailable\(''Legacy credential migration exceeds the bounded entry count\.''\);\s*$' 'pre-deduplication legacy migration bound'
  Require-Exact $source.windowsVault '^\s*async function readBoundedUtf8\(path: string, maximum: number\): Promise<string> \{\s*$' 'single-handle bounded legacy reader'
  ...
  Require-Exact $source.hostTests '^\s*test\(''classifies a vault write refusal as unavailable without reclassifying malformed registration input'',.*$' 'vault refusal classification test'
  Require-Exact $source.windowsVaultTests '^\s*test\(''persists the non-exported encryption key through a restart and binds ciphertext to AAD'',.*$' 'credential-vault restart and AAD test'
  Require-Exact $source.windowsVaultTests '^\s*test\(''shares one atomic master key across same-backend concurrent vault instances and restart decryption'',.*$' 'cross-instance master-key race regression'
  Require-Exact $source.windowsVaultTests '^\s*test\(''migrates a protected legacy entry only after Credential Manager readback, then removes only that legacy entry'',.*$' 'credential-vault legacy migration test'
  Require-Exact $source.windowsVaultTests '^\s*test\.skipIf\(process\.platform !== ''win32''\)\(''uses the actual Credential Manager backend under a disposable task namespace'',.*$' 'native Credential Manager integration test'
...
if ($SelfTest) {
  ...
  Assert-NegativeFixture $text vault 'class CredentialVaultUnavailableError' 'class CredentialVaultUnavailableError_REMOVED' 'Missing or duplicate exact contract (0): typed credential-vault unavailable result'
  Assert-NegativeFixture $text windowsVault 'export function createWindowsCredentialVault(' 'export function createWindowsCredentialVault_REMOVED(' 'Missing or duplicate exact contract (0): credential vault factory'
  Assert-NegativeFixture $text windowsVault 'CredWrite' 'CredWrite_REMOVED' 'Missing or duplicate exact contract (0): Credential Manager write boundary'
  Assert-NegativeFixture $text windowsVault 'migrateLegacyElectronVault(' 'migrateLegacyElectronVault_REMOVED(' 'Missing or duplicate exact contract (0): explicit legacy vault migration seam'
  Assert-NegativeFixture $text windowsVault 'child.kill(); fail();' 'child.kill_REMOVED(); fail();' 'Missing or duplicate exact contract (0): bounded Credential Manager helper deadline'
  Assert-NegativeFixture $text windowsVault '#masterKeyPromise ??= this.#createOrReadMasterKey();' '#masterKeyPromise_REMOVED ??= this.#createOrReadMasterKey();' 'Missing or duplicate exact contract (0): same-instance master-key singleflight'
  Assert-NegativeFixture $text windowsVault 'function decodeCanonicalBase64(' 'function decodeCanonicalBase64_REMOVED(' 'Missing or duplicate exact contract (0): canonical bounded credential decoder'
  Assert-NegativeFixture $text windowsVault 'spawn(executable,' 'spawn_REMOVED(executable,' 'Missing or duplicate exact contract (0): absolute PowerShell helper invocation'
  Assert-NegativeFixture $text windowsVault "inputStream.once('error'" "inputStream.once_REMOVED('error'" 'Missing or duplicate exact contract (0): helper stdin failure handling'
  Assert-NegativeFixture $text windowsVault 'readBoundedUtf8(' 'readBoundedUtf8_REMOVED(' 'Missing or duplicate exact contract (0): single-handle bounded legacy reader'
