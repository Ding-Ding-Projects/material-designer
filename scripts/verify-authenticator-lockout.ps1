...
function Validate-Sources([hashtable]$source) {
  ...
  Require-Exact $source.ladderUi '^\s*.*role="grid"[^>]*aria-label="Whack-a-mole board".*$' 'keyboard and screen-reader board'
  Require-Exact $source.ladderUi '^\s*.*aria-rowindex=\{.*\}.*$' 'mole grid row semantics'
  ...
  Require-Exact $source.ladderUi '^\s*.*bridge\.recordMoleHit\(lockoutId, challenge\.nonce, cell\).*$' 'per-click host mole route'
