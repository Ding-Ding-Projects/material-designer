...
@{ ok = $true; version = 4; rows = $ExpectedIds.Count; presentations = $Inventory.requiredCaptureVariants.Count; pinnedInputs = 1 + $ExpectedDependencies.Count; evidenceTargets = $ExpectedEvidenceTargets.Count; negativeCases = $Results.Count; sourceNegatives = 9; reparseFixtures = 1; results = $Results } | ConvertTo-Json -Depth 8
