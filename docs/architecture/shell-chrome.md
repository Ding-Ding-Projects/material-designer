...
Groups A through D keep the title bar, workspace tabs, status bar, and portalled
...
status-bar and screenshot-toast offsets consume the shared tokens. Full-height
surfaces subtract both chrome strips, and bounded overlays scroll internally.
Workspace and settings tab styles also use the reviewed shape and motion contracts.
