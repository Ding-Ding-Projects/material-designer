...
| File | The failure |
| --- | --- |
| [half-merged-features.md](half-merged-features.md) | The tree stopped compiling with a dozen unrelated-looking errors. One merge had integrated several features at one end and not the other: dropped import blocks, a duplicated `try` guard, a deleted method whose caller survived, and two generations of one contract. |
| [host-restoration.md](host-restoration.md) | A host type declaration, desktop preload, and runtime registration can advance while the structural validator still demands a retired bridge method. Keep the contract generation aligned and reject the obsolete shape explicitly. |
| [fileviewer-restoration.md](fileviewer-restoration.md) | The FileViewer retained consumers for capability, version-menu, and regex-search state that a partial merge had removed. |
| [site-startup.md](site-startup.md) | The documentation site stopped before first interaction because startup calls outlived their imports and a capped search count referred to a nonexistent aggregate. |
