...
If a deletion reports incomplete recovery, the original deletion error is preserved and the app reports that vault recovery did not finish. Retry after the credential vault is available. The app does not report a completed rollback or write an ordinary deletion-history record in that state.
