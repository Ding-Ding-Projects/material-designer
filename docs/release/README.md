...
| [release-assets.md](release-assets.md) | What each attached file is, which uploads go to the run rather than the release, and what is deliberately absent — no signature, no updater feed, no non-Windows artifacts. |
...
| What gets published? | A Windows installer, its SHA-256 file, a portable archive when one was built, and the code name's photograph. |
