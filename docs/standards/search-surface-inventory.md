...
| `desktop-file-viewer-version-download-menu` | FileViewer version download menu | `design/apps/web/src/components/FileViewer.tsx` | version-download-owned controller | wired at source level |
...
| `desktop-library` | Library search (`library-search`) | `design/apps/web/src/components/LibrarySection.tsx` | library-owned controller | wired at source level |
| `desktop-library-picker` | Library picker search (`library-picker-search`) | `design/apps/web/src/components/LibraryPicker.tsx` | picker-owned controller | wired at source level |
| `desktop-library-kind-filter` | Library kind filter (`library-kind-filter-search`) | `design/apps/web/src/components/LibrarySection.tsx` | filter-owned controller | wired at source level |
| `desktop-library-source-filter` | Library source filter (`library-source-filter-search`) | `design/apps/web/src/components/LibrarySection.tsx` | filter-owned controller | wired at source level |
| `desktop-library-design-system-menu` | Library design-system menu (`library-design-system-menu-search`) | `design/apps/web/src/components/LibrarySection.tsx` | menu-owned controller | wired at source level |
| `documentation-site` | Documentation content search | `site/index.html`, `site/assets/js/main.js` | input-keyed site controller | wired at source level |
| `documentation-settings` | Documentation settings search | `site/index.html`, `site/assets/js/main.js` | input-keyed site controller | wired at source level |
...
covered by this lane. Three required documentation tab-discovery rows
are deliberately marked RED because their product model is not present yet; the
tabs lane must wire them before the release check can turn green. Each wired row carries exact field ids, and reusable
