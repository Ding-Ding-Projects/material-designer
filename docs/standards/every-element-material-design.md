...
| Desktop modules reachable through static imports, local exports, re-exports, literal dynamic imports, and desktop source edges | 617 |
| Desktop component owners classified in reachable modules | 609 |
| Component owners proven reachable through parsed render references | 526 |
| Desktop JSX, fragment, portal, factory, imperative DOM, HTML assignment, shadow-root, registry-boundary, and render-prop nodes classified in reachable modules | 12,263 |
| Element nodes owned by render-reachable components | 11,339 |
| Desktop JavaScript or TypeScript source exclusions outside the entry-root graph | 88 |
| Desktop render-like comment exclusions | 432 |
| Static documentation-site HTML elements | 1,559 |
| Documentation-site JavaScript runtime creators | 406 |
| Documentation-site render-like comment exclusions | 20 |
| Explicit genuinely dynamic limits | 5 |

The 609 desktop owners contain 526 owners reached through parsed component
...
quietly disappearing. Their 924 element rows are similarly classified as
`module-reachable-only-element`. The remaining 11,339 element rows are proven
inside render-reachable owners. The complete 12,263-row source census breaks
down as follows:

| Parsed kind | Count |
| --- | ---: |
| Intrinsic JSX elements | 9,534 |
| Component JSX elements | 2,263 |
| Fragments | 212 |
| React portals | 81 |
| Imperative `createElement` calls, including aliases | 56 |
| Render-prop invocations | 39 |
...
The same rows record 7,417 conditional contexts, 1,862 map-produced contexts,
75 logical-expression contexts, and 185 spread-attribute elements. The collab
...
The 406 site runtime creators include 263 calls through local creator helpers,
34 direct `document.createElement` calls, 84 statically parsed `innerHTML`
template tags, 7 `document.createTextNode` calls, 11 dynamic `innerHTML`
