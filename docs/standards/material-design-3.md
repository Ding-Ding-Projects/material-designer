...
The current integrated source at `9a556bcfe` adds deterministic parity witnesses,
shell and overlay geometry repairs, settings and routine semantics, and a broader
shape and motion token migration. Full component anatomy and runtime visual proof
...
Groups A through E are integrated in the source tree. Group A repairs the
developer-only deterministic parity resolver, route witnesses, readiness ordering,
and bounded capture lifecycle. Library, Appearance, and Handoff have exact source
route mappings. Studio remains intentionally unresolved and fail-closed because no
semantically identical production destination is available. Group B aligns shell
chrome and overlay offsets with the shared geometry contract, keeps overlays within
the available viewport, and makes the direct-child and hit-surface checks exact.
Group C repairs settings and routine semantics, localized wrapping, target sizes,
and fixture resolution in focused source checks. Group D migrates the reviewed
shape and motion consumers onto shared Material Design tokens, preserves explicit
compatibility values, and uses a checked-in literal ledger. Group E makes declared
path comparison include upstream-only paths and adds a focused regression.
...
| Family | Source-level status at `9a556bcfe` | Anatomy and runtime status |
| --- | --- | --- |
...
| Requirement | Status |
| --- | --- |
| MD3 token layer in the application | **Source implemented and extended** at `9a556bcfe`. `md3-tokens.css` carries the colour roles, seeds, shape, elevation, motion, density, type, and compatibility contracts; `tokens.css` maps product names onto them. This remains source evidence, not rendered proof. |
...
| Density control | **Source implemented, runtime unverified.** Density values now drive shared primitive control dimensions and spacing; no installed display-scale or density matrix has been exercised. |
