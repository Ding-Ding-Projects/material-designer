...
| Strict appearance state and export schemas | **Source implemented.** `appearanceStateSchema.ts` validates bounded nested state documents, and `appearanceExportSchema.ts` validates the portable element schema with exact keys, UTF-8 limits, identity references, inheritance cycles and duplicate-key scanning before the renderer accepts an import. |
