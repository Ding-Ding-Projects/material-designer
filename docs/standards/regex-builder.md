...
[`ec2c76d7`](https://github.com/Ding-Ding-Projects/material-designer/commit/ec2c76d7) keeps that portalled builder in its own focus scope while the menu remains open. The current source lane carries independent builders for the inventoried desktop and documentation fields, including the Library picker, two filter pickers, the design-system menu, tab overflow, tab list, and tab context menu.
...
| Reachable from every inventoried search bar | **33 rows are hand-written, 30 are wired at source level, and 3 documentation tab rows remain explicitly RED** until the tabs lane supplies group, group-name, and master fields. Wired rows include exact stable ids for desktop, all nine actual FileViewer menus, Library picker/filter/menu fields, and documentation tab overflow/list/context-menu fields. Raw feature inputs outside this current route inventory remain open follow-up work. |
