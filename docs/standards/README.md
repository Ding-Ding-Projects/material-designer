...
| 3 | Runtime appearance customization | **Partial in source.** The application mounts the per-element editor boundary, target-specific appearance actions, the settings handoff, and the shared persistence/schema engine. Built interaction, complete per-element lock wiring, and capture evidence remain open. The site has its own source boundary with ownerless lock actions honestly unavailable until a provider is mounted. | [appearance-customization.md](appearance-customization.md) |
