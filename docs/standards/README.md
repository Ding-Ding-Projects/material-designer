...
| [universal-settings.md](universal-settings.md) | The shared local settings substrate for language, tone, narrator, schedules, attention modes, notifications, School mode, startup surprise, and status evidence. |
...
| 5 | Browser-style tabs everywhere | **Source-mounted in code.** The settings dialog has a 17-section tab strip, four-edge docking, viewport-bounded overflow, isolated menu regex search, vertical orientation keys and a portalled focus route; workspace pinning, grouping and the four app-wide discovery searches remain open. | [tabs.md](tabs.md) |
| 6 | Non-blocking notifications + centre | **Source-mounted in code.** The notification centre now has local search, selection, mark-read and dismiss bulk actions, export, and an explicit delete-capability boundary. Built interaction remains open. | [notifications.md](notifications.md) |
...
| 10 | Local version history | **Source-mounted web consumer.** The panel now composes date, action, domain and text filters, supports redacted exports, append-only restore proof and filtered bulk selection; daemon persistence and packaged interaction remain open. | [version-history.md](version-history.md) |
| 11 | Export everything, bulk actions everywhere | **Source-mounted.** The adapter catalogue keeps unavailable formats visible, faithful serializers validate output, and history and notification lists expose scoped bulk selection and export; product-wide integration and built interaction remain open. | [export-and-bulk-actions.md](export-and-bulk-actions.md) |
