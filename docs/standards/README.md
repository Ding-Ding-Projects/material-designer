...
| [status-hub.md](status-hub.md) | The evidence-backed status surface, authenticated acknowledgement boundary, local fallback, and C0/C2/C7/C12 mount contract. |
...
| 3 | Runtime appearance customization | **Partial source implementation.** Global appearance controls and the continuous picker exist, and the current lane adds a target registry, target-specific context actions, bounded anchored editor, layered state model, persistence, capability matrix, reset and append-only undo history. Built-artifact interaction, exhaustive per-element coverage and screenshot evidence remain open. | [appearance-customization.md](appearance-customization.md) |
