...
`components/unlock-ladder/UnlockLadder.tsx` is an accessible renderer surface for the host contract. It provides keyboard-reachable dish choices, ten labelled sum fields, a five-by-five mole grid, a text countdown tied to the host-provided round duration, reduced-motion handling, injected localized and funny copy, and explicit copy that winning clears the wait only. Reduced motion freezes the mole positions while retaining the numeric countdown and keyboard cell actions. Every mole click sends only its exact cell to the host. The host records its own time, visible-cell match, and one-hit state; the renderer never submits a client timestamp or hit list. It does not authenticate a user or create a session.
