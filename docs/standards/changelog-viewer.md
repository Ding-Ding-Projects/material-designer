...
**Status: source implemented, not yet audited in a running interface.** The
source-markdown path still reads `CHANGELOG.md` and
`design/docs/CHANGELOG/v*/en.md`, resolves its commit references against git,
and keeps unresolved or unrecorded commits visibly unlinked. The explicit
published-release path is generated by `scripts/generate-release-history.mjs`
into `design/apps/web/src/lib/changelog/release-history.generated.ts`.
That generated data currently contains 51 non-draft releases, each with its tag,
published timestamp, target commit, release URL, categorized notes, and full
40-character forge-backed commit URL. `--check` compares the committed data to
the current published inventory and fails on omission, duplication, a wrong SHA,
a stale date, or fabricated URL data.
The viewer is
`design/apps/web/src/components/changelog/ChangelogDialog.tsx`. Its typed open
event is source-ready, but the central C0/C12 mount is not wired in this lane,
so the dialog remains unreachable from Settings, About, and Help until that
integration lands.
...
Source checks cover the parser, commit resolution, filter composition, typed
dates, release-history completeness, and export rendering. **Nobody has yet
opened the viewer in a running build**, so its layout, keyboard path and
...
**Source checks are verified; the running interface is not.** Release-history
completeness, its red-then-green negative regression, focused date and Status Hub
source checks, release-integrity boundaries, and provenance boundaries are green.
The application has not been opened at the changelog mount in a running build, so
layout, keyboard, calendar, export and copy behaviour remain unverified by eye.
The central C0/C12 mount is not wired in this lane, so the dialog is unreachable
from Settings, About and Help. The documentation site does not implement the
in-app viewer.

Conformance requires all of:

- [x] every released version present, checked against the published release list
      rather than against the source file
- [x] every entry carrying a version, a date, categorized changes and a commit
      link
- [x] **every referenced commit proven to exist**, with the build failing on a
      dead link — proven by introducing a bad hash and watching the build fail
- [x] commit links resolving against the repository the build actually came from
