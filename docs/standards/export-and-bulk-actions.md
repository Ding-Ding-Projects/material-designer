...
> handoffs and the existing artifact exporters remain available. This lane adds
> the universal adapter catalogue, faithful local serializers, visible disabled
> entries for formats without bundled adapters, a shell-free VS Code handoff
> request, and selection bars on the notification and history lists. The full
