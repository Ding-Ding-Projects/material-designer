...
The loader does not import or own the universal-settings implementation. Its
desktop-facing seam accepts an injected `PersonalVocabularyC1` adapter with
synchronous `readSchoolMode` and live `subscribeSchoolMode` functions. Both
paths use `boolean | null`: `null` means the canonical host has not answered or
is unavailable, and remains a fail-closed state rather than being converted to
...
local settings projection as a fallback. The documentation-page module does
not inject a second key or event: it imports `STORAGE_KEY` and
`SCHOOL_MODE_EVENT` from `universal-settings.js`, then uses the imported
`readSchoolMode` and `subscribeSchoolMode` functions for its adapter seam.
...
owner and injects only its `readSchoolMode` and `subscribeSchoolMode` functions
into the personal-vocabulary module. The personal-vocabulary module imports
the canonical `STORAGE_KEY` and `SCHOOL_MODE_EVENT` itself. The feature does
not maintain a second School-mode key or event.
