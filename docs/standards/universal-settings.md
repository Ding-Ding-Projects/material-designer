...
mount; this lane does not edit `SettingsDialog.tsx`, the host protocol, the
preload bridge, or the documentation page. Until a host supplies those seams,
the state contract uses bounded browser-local storage and cross-tab events.
...
local-only and never labels an unrun capture or missing URL as verified.

## State and safety boundaries

The renderer state contract uses a versioned browser-local record and exposes a
feature-detected host bridge for a future app-data implementation. The
...
bridge. The host lane remains responsible for vault access, bounded response
reads, redirect refusal, and cancellation deadlines. External refreshes use a
generation check, so a late response cannot overwrite a newer state.
...
and recovery interaction remains owned by the dedicated toy-lock lane.
...
and `BroadcastChannel`. A host implementation can add redacted history
metadata and an isolated app-data history repository without changing the
renderer contract; those host responsibilities are intentionally outside this
lane.
...
The documentation page and its browser-storage equivalent remain outside this
lane. Any browser credential equivalent must be bounded, local to the current
visit, presence-only, and explicit about having no remote delivery or vault
claim.
...
surprise modules. The narrator now carries the selected stable voice identity,
rate, and pitch into its serialized speech environment. These are source claims
only until the host lane mounts the surface and hosted checks run.
...
The host-store, daemon unlock ladder, documentation page, and built-surface
capture records are outside this lane. They remain unrun here and must be
verified after the owning lanes mount and integrate their portions.
