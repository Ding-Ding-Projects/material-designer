...
- [ ] **Verify neutral publisher diagnostics on hosted release.** Source commit
      ...
      started.
- [x] **Record the C0 terminal source-preservation inventory.** The hand-written
      `docs/porting/c0-source-preservation.json` contains exactly 51 paths derived
      from `dd43dda7^..dfb5c168`, with 30 byte-identical rows and 21 intentional
      ...
      its 38 deliberate mutations all turn red before the restored inventory turns
      ...
      **廣東話進度：** C0 terminal source-preservation inventory 而家有 51 條 path，
      ...
      對 baseline、source-current 同 current tree 嘅 blob id 同 SHA-256，38 個
      deliberate mutation 全部先轉紅，four-space indented code row 唔會冒充
      active registration，active link 旁邊嘅 unrelated code
