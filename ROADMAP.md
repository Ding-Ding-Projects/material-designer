...
- [ ] **Drive every user-facing surface and retain one inspected capture after every interaction.** The source-only evidence boundary now fixes 30 features, two surfaces, ten destinations, and 70 scenes in a separate hand-written authority. Strict pre-conversion JSON admission and executed draft-2020-12 schemas cover the inventory, scene registry, ledger, receipts, artifact provenance, capture runs, every-element audits, and fixed approved-output manifests. No current-commit packaged artifact, real capture run, or per-interaction receipt exists yet, so the full drive remains unticked.
