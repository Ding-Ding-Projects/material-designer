...
| 14 | Accessibility and sizing as blockers | Every phase | **Partial, source-level audit only.** The 2026-08-06 pass fixed Figma field names and modal scrolling, and [`d918d33`](https://github.com/Ding-Ding-Projects/material-designer/commit/d918d33) makes the URL/notes names explicit through native `label`/`for` associations; the labels now use `useT` with the existing `dsCreate.*` catalog and English fallback. Context-menu label clipping and focus return, updater-dialog focus/reduced-motion behavior, and the design-system Back name are also covered. The 100/125/150/200% narrow-width bilingual runtime matrix and installed-build captures remain open |
