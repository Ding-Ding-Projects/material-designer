...
- [ ] **Drive every user-facing surface and retain one inspected capture after every interaction.** The source-only evidence boundary now fixes 30 features, two surfaces, ten destinations, and 70 scenes in a separate hand-written authority. Static JSON proves structure only and public append refuses; verified promotion belongs exclusively to the same production process that drives the approved cheap headless executable while holding its private capability and nonce. The committed live-driver registry and ledger are empty, no driver ran, and the full drive remains unticked.

      Commit [`c317c49f41b487fb7fede3f05101689b6f1950a8`](https://github.com/Ding-Ding-Projects/material-designer/commit/c317c49f41b487fb7fede3f05101689b6f1950a8) replaces the vulnerable foundation with version 3 receipts and a version 2 durable ledger. Every chain binds real Git objects, exact source and artifact commits, approved-driver run facts, the complete tuple including ordered `allowedOrigins`, original image limits, artifact provenance, an every-element audit, and one six-kind manifest under the canonical evidence root. The append path now uses a cross-process lock, bounded Windows sharing retry, atomic replacement, rollback, and final reopen and hash verification. The committed ledger is still empty and all captured status remains unclaimed.

      **廣東話進度：** Commit [`c317c49f41b487fb7fede3f05101689b6f1950a8`](https://github.com/Ding-Ding-Projects/material-designer/commit/c317c49f41b487fb7fede3f05101689b6f1950a8) 將舊 foundation 換成 version 3 receipt 同 version 2 durable ledger。每條 chain 都要綁實真 Git object、exact source 同 artifact commit、approved-driver run facts、包括 ordered `allowedOrigins` 嘅完整 tuple、原圖限制、artifact provenance、every-element audit，同 canonical evidence root 入面唯一一份六類 output manifest。Append path 而家有 cross-process lock、有限 Windows sharing retry、atomic replacement、rollback，同最後 reopen 加 hash 驗證。Committed ledger 依然係空，冇扮有 captured status。

      Commit [`867b3c75665db98c1c25894e27189b0c5b95b651`](https://github.com/Ding-Ding-Projects/material-designer/commit/867b3c75665db98c1c25894e27189b0c5b95b651) closes the last live-origin gap. The production orchestrator owns a nonserializable capability, same-session nonce, fixed executable and stdin bridge, dynamic process and window resolution, actual input, bounded semantic polls, fresh image time, private writer, atomic append, and `finally` revocation. Static validation reports `STRUCTURAL_ONLY` for nonempty evidence and cannot promote a row or scene. Thirteen valid schemas, 22 schema/admission negatives, 18 evidence negatives, 15 live-origin negatives, five privacy negatives, three reparse negatives, six sharing retries, and two cross-process lock writes passed under PowerShell 5.1 and PowerShell 7. The no-row production route invoked no driver and created no evidence root.

      Follow-up commit [`5887fbdaa2a24c601a44204db99d31a272fd94bd`](https://github.com/Ding-Ding-Projects/material-designer/commit/5887fbdaa2a24c601a44204db99d31a272fd94bd) puts the Git-admin cross-process lock inside the private capability-checked writer before it reads the next ledger sequence. The lock is released in `finally`; atomic replacement, bounded sharing retry, rollback, and final reopen proof remain unchanged.

      Commit [`ba381c8b190ad3803ed2f9631913d7421199850d`](https://github.com/Ding-Ding-Projects/material-designer/commit/ba381c8b190ad3803ed2f9631913d7421199850d) initializes exactly eight fixed no-reparse evidence parents before any output leaf is resolved, stages artifacts by SHA-256, creates only the exact run-image parent, and re-resolves every leaf before writing. It also adds committed `expectedPageUrl` bindings, exact normalized CDP target equality before every evaluation, and durable actual URL and digest binding. Both PowerShell engines pass the empty-root green fixture, five path/reparse negatives, and 18 live-origin negatives including same-origin wrong-path, foreign-URL, and redirected-URL substitutions. The no-row route still invokes no driver and creates no evidence root, so this roadmap item remains unticked.

      **廣東話進度：** Commit [`867b3c75665db98c1c25894e27189b0c5b95b651`](https://github.com/Ding-Ding-Projects/material-designer/commit/867b3c75665db98c1c25894e27189b0c5b95b651) 封咗最後一個 live-origin 窿。Production orchestrator 自己揸住 nonserializable capability、same-session nonce、固定 executable 同 stdin bridge、dynamic process 同 window resolution、真 input、有限 semantic poll、新鮮 image time、private writer、atomic append，同 `finally` revocation。Static validation 見到 nonempty evidence 只會報 `STRUCTURAL_ONLY`，冇權 promote row 或 scene。PowerShell 5.1 同 PowerShell 7 都通過 13 個 valid schema、22 個 schema/admission negative、18 個 evidence negative、15 個 live-origin negative、5 個 privacy negative、3 個 reparse negative、6 次 sharing retry，同兩個 cross-process lock write。No-row production route 冇叫 driver，亦冇建立 evidence root。
